...
records that generation in the lock owner record. Stale recovery requires an
expired lease and atomically renames the abandoned lock before acquiring a new
one and incrementing the generation. Immediately before ledger replacement, a
writer rereads both records and must still own the lock and match the current
sidecar generation or discard its bytes and retry. Missing, malformed,
...
| Persist fencing generation beside, not inside, authority          | Store one monotonic integer in an adjacent project-local fence sidecar. An exclusive first writer may create generation 1 only before any design-decision event exists; later absence fails closed. A lock holder increments the sidecar atomically, copies the generation into its owner record and decision event, and must match the lock and sidecar immediately before replacing the ledger. The sidecar coordinates writers but never grants approval.                                                                                                                                                                                                   | Derive a token from timestamps; derive it only from approval events; keep it only in the lock; append a lease event to the authority ledger                          | Timestamps are not a cross-process sequence; approval history may not change on every acquisition; deleting the lock loses an in-lock counter; lease events would mix transient coordination with durable human authority |
