...
authoritative feature design document that enters the bounded authority manifest
is a competing plan of record and blocks approval. Unlinked files outside that
manifest are not silently promoted into design authority.
...
prose-only convention. The cheapest discriminating slice is the R1
phase-transition scenario: an unresolved behavior-shaping choice must block
`plan-implementation → plan-execution`, while a current reviewed Implementation
Plan must permit it. A constant allow or deny implementation fails one of those
rows. Cross-host delivery remains the explicit `YCFFNC` prerequisite.
...
The Implementation Plan parser will enforce only observable structure and field
presence: required sections, a present non-empty architecture-at-a-glance
section, explicit applicability or skip markers, and the stable reference,
retrieval date, and applicable-version fields for each applicable external
evidence entry. It does not decide whether a stated model is complete, a skip is
honest, a status label is truthful, or a repair resolved the underlying choice.
The review rubric owns those semantic judgments, including decision depth,
evidence credibility and currency, persona consequences, truthful state,
focused 30–60 minute reviewability, and whether execution or evidence detail
obscures the decision. Structural and semantic findings are accumulated into
one receipt so authors repair the full current blocker set rather than one
hidden failure at a time.
...
project-local append-only review ledger. Only a canonical
`<timestamp> cli design-decision:<json>` line can carry authority, and its outer
timestamp must match the event timestamp; embedded markers in other event kinds
remain inert. A design-decision event records ticket, phase, exact Implementation
Plan digest, `approved` or `declined`, user-authority reference, append position,
and timestamp; `pending` is derived from configuration plus the
absence of a current matching approval and never masquerades as authority. The
source of truth is the current plan bytes plus the last valid matching event by
lock-serialized append position. Timestamp is audit evidence, never ordering
authority; a later appended decline supersedes an earlier approval even when
their clocks are equal or non-monotonic.
...
configuration.
Each written design-decision event also records the writer's fencing generation
as durability evidence; that field does not grant authority.
...
and atomically replaces the bytes before releasing it. A durable integer in an
adjacent project-local fence sidecar is the source of fencing generations; it
contains coordination state only, never approval authority. After exclusively
acquiring the lease-bearing lock, a writer atomically increments the sidecar and
records that generation, a random owner token, and process ID in the lock owner
record. Stale recovery requires both an expired lease and local operating-system
proof that the owner process is no longer alive. PID reuse may conservatively
look alive and delay recovery, but cannot authorize a replacement; if liveness
cannot be established, the lock is not stolen and the invocation fails closed as
pending. A permitted recovery atomically renames the
abandoned lock before acquiring a new one and incrementing the generation.
Immediately before ledger replacement, a writer rereads both records and must
still own the lock by its random token and match the current sidecar
generation or discard its bytes. Because a live owner is never preempted on the
supported single-host local-filesystem boundary, no reclaimed writer can enter
between that check and rename; cross-host or network-filesystem recovery is not
claimed. Missing, malformed,
non-monotonic, or unwritable fence state fails closed as pending. The only
initialization path is an exclusive first acquisition when the ledger contains
no prior design-decision event: create generation 1 with create-new semantics.
Once a design-decision event records a fencing generation, an absent sidecar is
data loss and cannot be recreated from timestamps. Contention timeout fails
closed as pending and writes no approval event. This guarantee assumes the documented
...
idempotency key records the decision identity and the append position it
supersedes for audit; retry deduplication compares the requested identity with
the current serialized decision while holding the lock. The gate rereads the
...
leaves the phase blocked. Distinct concurrent design-decision appends cannot
overwrite each other; a failed writer may reread and retry only inside its
bounded contention budget. Timeout expiry is terminal-pending for that
invocation and performs no later retry or approval write.
...
readable rather than making the whole ledger unusable. This establishes event-kind
forward compatibility only. Mixed-version safety additionally requires
`YCFFNC` to move every installed writer onto the shared lock/fence protocol
before activation. Rolling the feature back
...
| R13 structural-vs-semantic evidence                      | Installed Safeword CLI running the Implementation Plan parser followed by the packaged semantic reviewer contract                                                                  | Real-process CLI integration plus a parser matrix that independently removes the evidence reference, applicable version, and retrieval date; contract mutation and review fixture for a contradicted applicability skip                                                    | Each missing field produces its own structural finding, while a structurally complete but dishonest skip reaches semantic review and fails there. The fixtures prove the division of responsibility and installed wiring, not external-source truth or live-reviewer consistency.                                                                                                                                                                                                                                                                                                                                                                    |
| R3 mental model and focused reviewability                | Implementation Plan parser followed by the real review coordinator with bounded project-local packet                                                                               | Parser rejection for an absent or empty architecture-at-a-glance section; deterministic review fixtures for usable decision-first, unusable contract-bearing, execution-first, evidence-ledger-first, and wholly absent load-bearing-decision presentations; current-revision independent-review acceptance artifact | Structural absence blocks before semantic review with the same observable no-approval outcome; a present section still fails semantic review when it does not give a usable mental model, omits a load-bearing decision, or buries decisions under execution or evidence detail. Live reviewer consistency remains variable, so unavailable exact-contract acceptance is reported honestly rather than inferred.                                                                                                                                                                                                                                           |
...
| R5 architecture and R7–R9 durable/single-record behavior | Implementation Plan parser and semantic review using configured project knowledge; installed Safeword CLI planning edit gate for the configured architecture location              | Unit contract matrices plus integration review packet and edit-gate matrix covering an exact configured file; a sibling; a direct `YYYYMMDD-slug.md` child of a configured ADR directory; non-dated, nested, outside-directory, resolved-target escape, and ordinary-path denials | A configured file admits only itself. A configured directory admits only direct dated children whose resolved targets remain inside it. Semantic review still blocks absent or unresolved links, competing design authority, and significant choices without a durable record.                                                                                                                                                                                                                                                                                                                                                                       |
| R6 decision-bearing data coverage                        | Packaged plan contract and semantic reviewer over applicable, honestly inapplicable, incomplete, coherent, contradictory, and mechanics-polluted data models                       | Contract mutation matrix plus deterministic semantic-review integration                                                                                                                                                                                                   | The proof varies all eleven decision areas, permits a persisted owner that agrees with the named source-of-truth authority, rejects a contradictory owner, and rejects exact migration commands even when every data decision is present. It proves that decisions and mechanics are distinguished, not that a future migration will execute correctly.                                                                                                                                                                                                                                                                                                |
...
| R17 significant workflow decision depth                  | Packaged plan contract and deterministic semantic-review conformance boundary                                                                                                      | Contract matrix plus installed CLI review integration                                                                                                                                                                                                                     | Durable state, authorization, concurrency, lifecycle, migration, and compatibility families each pair a complete decision model with a family-specific omission, varying state, transition authority, atomicity or crash boundary, retry, compatibility, and preserved evidence so labels alone cannot pass.                                                                                                                                                                                                                                                                                                                                         |
| R18 measurement-design ownership                         | Product promise → Implementation Plan review packet                                                                                                                                | Contract matrix plus deterministic semantic review                                                                                                                                                                                                                        | Product owns the promised 30–60 minute target and reviewer population. This plan owns the study method and validity safeguards: start when the reviewer begins the decision summary, stop at approve/block, record whether the reviewer can explain the approach and unresolved authority, sample reviewers without authoring context, and report setup interruptions separately. Rejection cases cover an unresolved validity safeguard, changed Product target, changed population, and substituted instrumentation commands. When Product makes no quantitative promise, missing applicability and bare skips block while a justified skip passes. The receipt's pass/fail is diagnostic input, not duration proof by self-report. |
| R19 approval-ledger durability                           | Project-local shared decision writer and installed transition gate using real temporary-project files, concurrent approval-decision writer processes, a controlled clock, and injected lock/fence/write/phase faults | Concurrent-decision integration plus crash/fault matrix covering distinct decisions, superseded retries, first sidecar initialization, missing or malformed sidecar after a decision, contention timeout, expired lease with dead or still-live owner, stale fencing token, crash before event write, crash after event write but before phase change, idempotent retry, unknown-event preservation, and an embedded decision marker inside an unknown event | The proof fails if a decision append is lost, a live owner is preempted, a stale writer commits, missing fence state is rebuilt after authority exists, timeout mutates bytes, recovery invents approval, retry duplicates an event, an unknown event changes bytes, or an embedded marker gains authority. Unifying other ledger writers is a `YCFFNC` migration prerequisite. This establishes the supported single-host local-filesystem boundary only; installed-host and non-local filesystem claims require their own acceptance evidence. |
| R19 exact-plan human approval and headless completion    | Installed CLI approval boundary; interactive accept, decline, approver-available, and review-blocked no-prompt paths cross a real terminal/PTY boundary, while non-interactive paths settle through real processes | Real-process integration with terminal input/output for every prompt or no-prompt claim                                                                                                                                                                                   | Approval runs only after semantic review passes, binds exact bytes, recomputes currency after edits, records decline without advancing, and leaves headless work settled in Implementation Planning with approval pending. The two post-Execution-Plan scenarios use a contract-compatible completed-plan fixture to prove approval reuse and staleness here; `7CAMAD` must prove composition with the real artifact and implement-entry gate before parent release. Fixture authority proves consumer behavior only; authentic human-event provenance remains a `5F5ZZA` release prerequisite.                                                          |
| R20 complete repair loop                                 | Installed CLI review and decision-discovery loop with deterministic reviewer process results                                                                                       | End-to-end CLI integration                                                                                                                                                                                                                                                | The first receipt exposes the full three-defect set together; accepted owners resolve each decision; the approving receipt binds the corrected digest and cannot bind the original bytes; a second review that still omits one authorization decision remains blocked; unavailable external authority produces an honest pending result rather than approval or a fixed retry cap.                                                                                                                                                                                                                                                                  |
...
| Make durable architecture recording possible before approval      | A configured architecture file admits only that exact file; a configured ADR directory admits only direct `YYYYMMDD-slug.md` children whose resolved targets remain inside it. Siblings, non-dated children, nested/outside paths, resolved-target escapes, and ordinary files remain frozen.                                                                                                                                                                                                                                            | Exact-path-only directory matching; textual prefix matching; permit every directory child; record after implementation starts                                       | Exact-only prevents ADR creation; textual prefixes permit path escapes; blanket child access widens the freeze; post-approval recording makes R7 circular                                                            |
...
| Persist approval state safely                                     | Reuse the project-local append-only review ledger with digest-bound, idempotent approval/decline events; derive pending and currency from config, current bytes, and the last valid event by serialized append position; atomically replace ledger bytes before phase transition. Preserve unknown event kinds as authority-inert bytes so known receipts remain readable. Mixed-version writers are safe only after `YCFFNC` migrates every writer to the shared lock/fence protocol.                                                                                                                                                              | Store mutable approval on ticket frontmatter; overwrite one current receipt; order by timestamp; keep state only in host memory; reject the whole ledger when one event kind is unknown | Frontmatter conflates workflow and authority; overwrite destroys history; clocks are not authority order; host memory breaks handoff; whole-ledger rejection lets one extension erase valid evidence |
| Persist fencing generation beside, not inside, authority          | Use the sidecar/owner-record protocol defined in Approach. Reclaim only after lease expiry and proof that the owner PID is dead; ambiguous liveness fails closed. The sidecar coordinates writers but never grants approval.                                                                                                                                                                                                                                                                                                                                                                                                                                    | Recover on lease expiry alone; derive a token from timestamps or approval events; keep it only in the lock; append a lease event to the authority ledger             | Lease-only recovery can preempt a paused live writer; timestamps and approval history are not an acquisition sequence; deleting the lock loses an in-lock counter; lease events would mix transient coordination with durable human authority |
...
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| Optimize for the NTB without constraining the TBU | Blocked receipts lead with a plain recovery while preserving paths, failed checks, identities, and evidence for technical readers                 | [R15 receipt scenarios](features/approve-coherent-implementation-plans.feature)                                                                                                                                                                 |          |
| Structure enforces; instructions suggest          | Explicit artifacts, phase transitions, contract identities, and receipts enforce the boundary; prose teaches the qualitative judgment             | [phase and plan transition integration](packages/cli/tests/integration/plan-transition-gate.test.ts)                                                                                                                                            |          |
| Fire at boundaries, not every turn                | Contract and semantic review run at Implementation Planning exit; no per-turn plan-quality judge is added                                         | [review-trigger boundary coverage](packages/cli/tests/hooks/review-trigger.test.ts)                                                                                                                                                              |          |
| Add, never replace                                | Preserve customer-owned plan bytes and do not enable the new phase sequence until sibling `YCFFNC` supplies deterministic in-flight compatibility | [schema ownership regression coverage](packages/cli/tests/schema.test.ts); `YCFFNC` migration scenarios remain a release prerequisite                                                                                                            |          |
| Discover decisions before prescribing work        | Implementation Planning resolves consequential choices; Execution Planning may only sequence them                                                 | [R11 and R14 accepted behavior](features/approve-coherent-implementation-plans.feature)                                                                                                                                                          |          |
| Contribute, then converge                         | The planning guidance proposes concrete candidates and trade-offs before seeking missing user-only authority                                      | [planning skill contract coverage](packages/cli/tests/skills/bdd-review-evidence-contract.test.ts)                                                                                                                                              |          |
| Correct and safe; then clear; then simple         | One new phase reuses the existing transition/review infrastructure; no second workflow engine or dependency is introduced                         | [phase parity and transition coverage](packages/cli/tests/integration/plan-transition-gate.test.ts)                                                                                                                                             |          |
...
- The accepted ledger's blanket untagged-scenario note predates the parser and
  semantic-review split. For scenarios whose accepted result explicitly names a
  structural check, RED/GREEN must exercise the production parser; the exact
  packaged-contract mutation boundary applies to semantic verdicts. This is a
  clarification of proof ownership, not permission to use scripted verdicts.
...
- A focused-review receipt names execution or evidence detail as obscuring the
  choices, or the separate Product-owned review study exceeds its 30–60 minute
  window: tighten the summary or move supporting detail behind links; do not
  move unresolved decisions into Execution Planning.
