...
reference, append position, and timestamp; `pending` is derived from configuration plus the
...
other; a failed writer may reread and retry only inside its bounded contention
budget. Timeout expiry is terminal-pending for that invocation and performs no
later retry or approval write.
...
| R19 approval-ledger durability                           | Project-local shared ledger writer and installed transition gate using real temporary-project files, concurrent approval and review-stamp writer processes, a controlled clock, and injected lock/fence/write/phase faults | Concurrent-writer integration plus crash/fault matrix covering distinct and mixed event appends, first sidecar initialization, missing or malformed sidecar after a decision, contention timeout, expired lease with dead or still-live owner, stale fencing token, crash before event write, crash after event write but before phase change, idempotent retry, and unknown-event preservation | The proof fails if an append is lost, a live owner is preempted, a stale writer commits, missing fence state is rebuilt after authority exists, timeout mutates bytes, recovery invents approval, retry duplicates an event, or a known append changes an unknown event's bytes. It establishes the supported single-host local-filesystem boundary only; installed-host and non-local filesystem claims require their own acceptance evidence. |
