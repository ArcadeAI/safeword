...
The first concrete consumer — the **reviewer tier** — is resolved and shipped (V2 below): provider-neutral, guidance-based, no model names. The remaining open items are now closed:

- **Per-sub-agent tier mapping + Mechanism — handed to [9BDDGP](../9BDDGP-dynamic-workflows-for-safeword/ticket.md) (dynamic-workflows).** The actionable intent already shipped as guidance — reviewers = different + no-weaker; producers (Explore / refactor scout / figure-it-out research) = cheaper/varied is fine. A real mechanism that _sets_ a model per sub-agent at spawn time can't live in safeword guidance (provider-neutral — safeword can't name models or map tiers); that's 9BDDGP's job (it picks model per agent). No standalone tier-mapping research remains here.
- **Gate-enforcement of "no weaker" — won't-do.** V2 removed the tier ladder; enforcing "no weaker" in the `crossModelReview` hook would require a hardcoded model-tier table — exactly what the provider-neutral principle forbids (rots each release, false-blocks unknown ids, binds to one provider). The gate enforces **different** only (all it can do model-agnostically); "no weaker" stays guidance by design.
- **Frontier-cost / calibration** were model-economics framing that the tier abstraction + harness-owned resolution make moot at the safeword layer — revisit under 9BDDGP only if real per-agent selection lands.

**Status:** deliverable shipped in PR #283 (de-model + provider-neutral learning + V2 reviewer-model wording). Ready to close once #283 merges; any future mechanism work lives in 9BDDGP.

## V2 — lean reviewer-model wording (shipped 2026-06-20)
...
- 2026-06-20 /figure-it-out: which other sub-agents inherit the "different + no-weaker" rule. The rule has two halves that generalize differently: **different model** (decorrelate the author's blind spots) applies to _review of the author's own work_; the **no-weaker floor** (asymmetry collapses on hard-to-verify bugs) applies wherever that does.
  ...
  - **Closed — won't-do (gate-enforce no-weaker):** kept as _guidance_, not gate enforcement. Hardcoding a tier ranking of model ids into the `crossModelReview` stop-hook would rot every release, false-block unknown ids, and bind to one provider — exactly the brittleness this ticket forbids. V2 then removed the tier ladder entirely, so there is nothing left to enforce; the gate checks **different** only. Decision: do not implement.
...
- 2026-06-20 V2 wording approved ("go forth") → **shipped** across quality-review + both BDD gates (3 mirrors each); fresh-context review caught a MEDIUM (bdd/SKILL.md scenario-gate lacked TDD.md's `skip:` escape) — fixed; verified (696 tests, lint, build, parity); audit clean; CI green on #283.
- 2026-06-20 **Closed remaining scope** (user: "close them both out"). (1) Per-sub-agent tier mapping + Mechanism → handed to 9BDDGP (dynamic-workflows owns per-agent model selection; safeword can't set models provider-neutrally — actionable intent already shipped as guidance). (2) Gate-enforcement of no-weaker → **won't-do** (V2 left no tier ranking to enforce; a model-tier table violates provider-neutrality). Ticket deliverable is shipped in #283; ready to close on merge.
