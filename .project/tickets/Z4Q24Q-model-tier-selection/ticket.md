...
**Provider-neutral by design.** Tiers are abstract capability classes, **never specific model names**. Safeword ships to multiple agent providers (Claude Code, Codex, Cursor, …), each with its own models that change release to release — naming a model in shipped guidance or normative policy is brittle and provider-specific. Express tiers; let the harness/provider resolve tier → concrete model at runtime. Any model names anywhere in this ticket are dated, illustrative snapshots only and must never reach a shipped skill.
