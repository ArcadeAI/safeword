...
- 2026-06-23 For the Opus-4.8 verifier-fan-out work (9BDDGP / SKQR0G): re-confirmed Cursor has subagents (`subagentStart/Stop`, Task tool) and is **auto-spawn capable**, unlike Codex (user-invoked only). One gap relevant to P30CRP (SAFEWORD.md reinjection): Cursor's `preCompact` is **observe-only and cannot inject post-compaction context** (only `sessionStart`/`postToolUse` inject `additional_context`), so compaction-resistant re-injection is weaker on Cursor than on CC/Codex (both have `PreCompact`+`PostCompact`). No change to this epic's enforcement tickets.
