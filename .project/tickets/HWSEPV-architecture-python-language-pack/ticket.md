...
scope:
  ...
  - Python dependency fingerprint — the shape-fingerprint's dependency set includes the `[project] dependencies` PEP 508 specifier NAMES (the leading distribution name, version/extras/markers stripped), so Python dependency drift moves the package's fingerprint. (`[project.optional-dependencies]` — a TABLE of extra→array, a different shape than the single `dependencies` array — is deferred to the follow-up below; a dep declared only under an extra is honestly not counted yet.)
