...
| Default categories and typed shape        | Unit mutation tests plus real plan-execution admission cases remove each category and require every omission in canonical order; a generated Execution Plan is parsed through the same validator with all eleven categories under both `designApprovalGate` settings; a testing category backed only by review receipts is rejected until at least one contributor item requires a real-boundary command proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Structure cannot prove that an obligation is semantically applicable                                                                                                                                                           |
...
| Contributor obligations cannot be escaped | Integration: mutate each retained item field separately—ID, Category, Obligation, Owner, and Required proof—plus item insertion/removal, every proof-specification field, reviewed applicability, and live `designApprovalGate`; readiness, proof recording, the atomic update path, and the first-execution consumer all return `stable_definition_changed` until re-review                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | The shared ledger has concurrency integrity, not authenticated authorship; 5F5ZZA owns authenticated host-user provenance                                                                                                      |
...
`compatible_earlier_allowed`, but an independent `delivery-compatibility`
review must
...
Git diff from the producing revision to the reviewed revision. The dedicated
review kind reuses the existing coordinator, job integrity, reviewer routing,
and verdict envelope. A retry finds the matching
...
The reviewer answers one question: _Does the earlier passing receipt still
establish this retained proof boundary at the reviewed revision?_ It reasons
only from the exact receipt identity, retained proof definition, contributor
reason, revision pair, and complete bounded diff in the trusted packet. It must
request changes when the reason does not cover every changed hunk; when code,
tests, fixtures, command inputs, configuration, or dependencies used by the
proof changed; when the diff contradicts the reason; or when the packet cannot
show a complete reviewable delta. Approval means the diff is irrelevant to the
named proof boundary, not merely that the request is well formed.

**Alternative considered:** Trust the contributor's reason; re-review the whole
Execution Plan; use the generic `quality-review` rubric; or add an open-ended
custom review-question field to the shared packet.
...
ID moves identity matching onto the user, the generic rubric can approve a
well-formed request without judging compatibility, and a custom-question field
broadens the shared packet contract beyond this concrete need.
...
claim. At least one contributor-owned item in the `testing` category must
require a `command` proof reviewed as `real_boundary`; otherwise admission
returns `testing_proof_not_executable`. Planning-review receipts may support
planning-derived categories, but cannot by themselves make a contribution
complete before executable testing exists.
...
- Reusing an earlier-revision receipt uses
  ...
  declares `compatible_earlier_allowed` and an independent
  `delivery-compatibility` review accepts that reason
