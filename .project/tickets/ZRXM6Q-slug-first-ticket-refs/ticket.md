...
- 2026-06-06T23:50:00Z Verified — build ✓, lint (eslint+tsc) ✓, parity ✓ (120 pairs / 3 contracts in sync), full suite ✓ (2491 pass / 1 skip / 0 fail). Slug-first now live at: `ticket new` confirmation, INDEX rows, `check` advisories, compact-context hook; helper + invariant test lock the `label (ID)` format; SAFEWORD.md "Talking to the user" carries the naming rule. Commits 943bc2e9 (CLI sites), 5f0413ec (compact-context), 1f4d7324 (guard + rule), plus f1ddf512 (helper). **Open follow-up:** the per-turn prompt-questions active-ticket line (deferred — needs a noise-calibration pass). Staying `in_progress` pending the user's call on that follow-up + close (no verify.md yet).
- 2026-06-07T01:20:00Z Follow-up done (user: "do it") — added the per-turn prompt-hook line `- Ticket: slug (ID)` before the phase reminder, only when a ticket is active; slug derived from the folder, inline (two hook sites, no mirror lib). New test phase-derivation 1.3 locks it. Full suite re-run: 2491 pass / 1 skip; the single fail was the timing benchmark in `technical-constraints.test.ts` under load — confirmed green 3/3 in isolation, unrelated to this change. Commit b7fda962. **Every ZRXM6Q surface is now slug-first.** Remaining to close: `/verify` → verify.md → status done (awaiting go).
- 2026-06-07T02:35:00Z Quality-review follow-ups (user: "make the improvements"). Consolidated the slug derivation into `getTicketInfo` (frontmatter `slug:` ?? folder split); prompt-hook + compact-context now both read `ticketInfo.slug` — removes the prompt-hook's inline folder-split (note 1) and unifies the hook label to the short slug (note 2; compact-context was title). `check.ts`'s CLI-side split left as-is — different world, lone site (cross-world sharing is the mirror-lib bloat figure-it-out rejected). Locked the derivation in active-ticket-lookup.test.ts (+2 assertions); made `slug?` optional so `resolveStopPhase`'s partial TicketDetails still typechecks. Commit d78c04d1. Full suite **2492 pass / 1 skip / 0 fail** (clean — earlier flaky perf passed too).
- 2026-06-07T02:50:00Z /quality-review on the full ticket found a **missed surface**: stop-quality.ts:472 named the NEXT ticket by bare ID ("begin work on ${next.ticketId}") in the hierarchy-navigation message — agent-facing, in scope. Fixed by reusing `getTicketInfo(projectDir, next.ticketId).slug` → slug-first (commit 7758663f). Strengthened the compact-context test to assert the exact `Ticket: test-ticket (099)` format. Full suite re-run **2492 pass / 1 skip / 0 fail**. Acknowledged gap (pre-existing, heavy to test cheaply): the check coverage-advisory slug format + the stop-navigate message have no dedicated format assertion — both reuse derivations that ARE tested (getTicketInfo.slug, formatTicketReference).
