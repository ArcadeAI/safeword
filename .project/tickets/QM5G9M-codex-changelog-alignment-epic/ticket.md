...
- 2026-06-23 Read the Codex **Subagents** page (flagged above as not-yet-read end-to-end) for the verifier/fan-out use case — feeds 9BDDGP / SKQR0G (Opus-4.8 fresh-context-verifier amplification). Findings: subagents support **parallel fan-out** ("Codex waits until all requested results are available, then returns a consolidated response") and **per-subagent model** (`model` + `model_reasoning_effort`, inherit-from-parent if omitted), but are **user-invoked only** ("Codex only spawns a new agent when you explicitly ask it to") with `agents.max_depth=1` (deeper nesting configurable). Implication: cross-model fresh-context review is expressible on Codex, but an *autonomous* verifier loop can't self-spawn — it degrades to a user-invoked step. No change to this epic's gate findings; closes the subagents-doc research gap.
