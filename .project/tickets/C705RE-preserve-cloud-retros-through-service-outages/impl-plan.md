...
1. Before resuming implementation, identify a provider-native carrier that is
   available by default in the normal Cloud auto-mode environment. Claude
   Cloud's exposed delayed-message tool is not such a carrier: its permission
   classifier denied registration twice, including after chat approval. Any
   future one-question spike must prove the real `POST /v1/public-retros`
   receipt path without delaying the task. Claude, Codex Cloud, and Cursor
   Cloud Agents all remain `skip: live receipt proof outstanding`.
...
| Carrier rollout | Defer every public-cloud release until a provider proves a default-enabled, detached, receipt-confirmed carrier | Build a receiver before its sender; raise the synchronous Stop-hook timeout; add a Cloud permission exception | The first has no end-to-end value and widens public ingress; the second violates the invisible contract; the third turns zero-setup delivery into a user/harness configuration requirement. |
...
local probe of the same Railway endpoint completed in 311 ms. Its only examined
detached alternative, `send_later`, has one-minute granularity and was denied
by auto-mode before registration twice; in-chat approval could not alter that
harness rule. The existing architecture keeps unauthenticated records separate
from the private filing worker. Receiver-first was close because the
SQLite/keyring design is small, but loses because it creates no usable handoff.
The current decision is to defer the whole public-cloud slice until a real
sender is proven. Premortem: carrier research could become an open-ended
platform effort; cap each provider to a one-question spike and stop after the
first failed proof.
...
| --- | --- | --- | --- |
| Structure enforces; instructions suggest | A provider route cannot exist until its hosted evidence and default auto-mode permission exist; no public HTTP route is deployed first. | The recorded Cloud spikes and a future real-receipt proof are release gates. | |
...
- A provider proves a detached carrier with a real public-ingest receipt in its
  default runtime permission mode: create a fresh plan for that provider's one
...
- A provider requires a project permission rule or non-default mode: return to
  behavior definition; that changes the zero-setup contract rather than merely
