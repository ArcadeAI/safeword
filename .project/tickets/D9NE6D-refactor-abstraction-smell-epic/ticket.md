...
- 2026-06-19 Implemented R1/R2/R3 in the refactor template: R1 adds "Wrong level of abstraction" to the smells list + a Tier 3 "Generalize the Mechanism" catalog row (detect via shotgun surgery); R2 adds an optional "Scout first" that runs `/lint`+`/audit` for mechanical smells and reserves prose judgment for the semantic ones (reuse, altitude), prioritized + one-at-a-time + caps-record; R3 adds a Phase 4 regression checklist (dropped guard / setup-teardown asymmetry / predicate side effect / flipped config default). Iron law untouched. Synced byte-parity → `.claude` + `.agents`; condensed into the Cursor `.mdc` (+ dogfood). parity 157; tests green (596). Status in_progress pending `/verify` + user confirmation.
- 2026-06-19 Epic-review nit: the Cursor catalog was missing the "Complex loop → Replace Loop with Pipeline" Tier-3 row (pre-existing 1-row drift — 13/14 rows mirrored SKILL.md exactly, so drift not condensation). Added it; Cursor catalog now 14/14 matches SKILL.md.
