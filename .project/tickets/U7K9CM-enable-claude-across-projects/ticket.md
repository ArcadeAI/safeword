...
2026-09-06 UTC: Rebased onto origin/main f3a25692f. Regenerated current Claude/Codex plugin bundles; 55/55 targeted tests, 102/102 affected scenarios (4,787 steps), and the Claude plugin release contract pass. Main added six user-facing documentation surfaces that still described project scope as the default; updated all six and added a cross-surface documentation contract. Documentation tests pass 11/11 and the website production build succeeds.

2026-09-06 UTC: Retried the retained Tart VM with the rebased package. Claude remains authenticated, but no independent reviewer is usable: the isolated Codex profile is unauthenticated, OpenCode is absent, and Claude remains incompatible with the root-managed enterprise MCP configuration. The guest also has insufficient free disk for npm to replace the test CLI (`ENOSPC`). No review receipt was fabricated and the full BDD lifecycle remains pending in a fresh/provisioned VM.
