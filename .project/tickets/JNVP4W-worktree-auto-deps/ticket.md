...
- 2026-06-24T03:52:00.000Z **Priority bump — now has production evidence.** A Claude-Code-on-web session committed before deps were installed; husky/lint-staged was inactive (no `node_modules` → no git hooks), so an **unformatted file landed and CI `format:check` failed** (PR #351), costing a red run + fixup commit. This is the exact failure this ticket prevents, observed in the wild — promote off backlog and build next. The whole pre-commit guard chain (format, version-parity, parity-contracts, ticket-id, dogfood-direction) is silently absent in this window, so the blast radius is broader than formatting alone. Tracked externally as GitHub issue #364. Plan above is converged and unchanged; no re-design needed.
