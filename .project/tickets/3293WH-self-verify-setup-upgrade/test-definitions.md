...
> Rationale: TB1.AC1 — a mutating command verifies what it wrote at the
> moment it exits, where the breakage actually is.

### Scenario: self-verify-setup-upgrade.TB1.AC1.clean_setup_ends_with_health_verification
...
### Scenario: self-verify-setup-upgrade.TB1.AC1.setup_with_post_run_issues_exits_nonzero
...
> Rationale: TB1.AC1 — `SAFEWORD_SKIP_INSTALL` is an existing "don't install"
...
### Scenario: self-verify-setup-upgrade.TB1.AC1.skipped_install_setup_does_not_fault_absent_packages
...
> Rationale: TB1.AC2 — same failure semantics as setup; report-and-fail, no
> repair loop (reconcile already ran — an issue it couldn't fix won't be fixed
> by running it again).

### Scenario: self-verify-setup-upgrade.TB1.AC2.clean_upgrade_ends_with_health_verification
...
### Scenario: self-verify-setup-upgrade.TB1.AC2.upgrade_with_post_run_issues_exits_nonzero
...
> Rationale: TB1.AC3 — an "update available" nag right after upgrading is
...
### Scenario: self-verify-setup-upgrade.TB1.AC3.setup_health_verification_carries_no_update_check
...
### Scenario: self-verify-setup-upgrade.TB1.AC3.upgrade_health_verification_carries_no_update_check
...
### Scenario: self-verify-setup-upgrade.TB1.AC3.health_module_has_no_update_check_path
...
> Rationale: TB1.AC4 — the happy path must not get noisier; one brief success
...
### Scenario: self-verify-setup-upgrade.TB1.AC4.clean_setup_prints_one_health_summary
...
### Scenario: self-verify-setup-upgrade.TB1.AC4.clean_upgrade_prints_one_health_summary
...
### Scenario: self-verify-setup-upgrade.TB1.AC4.advisories_surface_once_without_failing
...
> Rationale: TB1.AC5 — "run `safeword upgrade` to repair" printed by upgrade
...
### Scenario Outline: self-verify-setup-upgrade.TB1.AC5.post_upgrade_failure_hint_omits_run_upgrade
...
### Scenario: self-verify-setup-upgrade.TB1.AC5.standalone_check_keeps_existing_hint
...
> Rationale: TB2.AC1 + TB2.AC2 — the doctor-style diagnostic stays public
> for CI/debugging; humans just stop being told to run it routinely.

### Scenario: self-verify-setup-upgrade.TB2.AC1.docs_present_check_as_automatic_first
...
### Scenario: self-verify-setup-upgrade.TB2.AC2.standalone_check_behavior_unchanged
