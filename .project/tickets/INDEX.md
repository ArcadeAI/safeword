...
- **New /build-signals skill — four implementation options, mandatory feature tag, in-code instrumentation discipline (1W107W)** (in_progress, epic: bdd-phase-three-merge)
  ...
  blocks: Breach issue projection — signal breach → tracker issue via the ticket bridge (K51FYZ)
...
- **Epic: PM-grade intake protocol (169)** (open, epic: pm-grade-intake)
...
### ticket-anchor-external-bridge

- **Ticket bridge — one-way provider-adapter layer (Linear/GitHub/Jira via Arcade MCP) (JS5K5G)** (in_progress, epic: ticket-anchor-external-bridge)
  A single, **one-way** (file → tracker) provider-adapter layer that lets safeword write/update issues in the customer's real ticket system — Linear, GitHub Issues, or Jira — routed through Arcade.dev's MCP servers (per-provider OAuth handled there). This is the shared spine the coordination mirror and the breach-router both build on; neither defines its own adapter.
  blocks: Breach issue projection — signal breach → tracker issue via the ticket bridge (K51FYZ), Optional one-way coordination mirror to Linear/GitHub/Jira (consumes the ticket bridge) (THSPA5)
  → `.project/tickets/JS5K5G-ticket-bridge`
- **Breach issue projection — signal breach → tracker issue via the ticket bridge (K51FYZ)** (in_progress, epic: ticket-anchor-external-bridge)
  When a signal breaches its threshold, project that breach as a structured issue in the customer's tracker via the [ticket bridge](../JS5K5G-ticket-bridge/ticket.md) (JS5K5G), so breaches get triaged instead of firing into the void.
  blocked by: Ticket bridge — one-way provider-adapter layer (Linear/GitHub/Jira via Arcade MCP) (JS5K5G), New /build-signals skill — four implementation options, mandatory feature tag, in-code instrumentation discipline (1W107W)
  → `.project/tickets/K51FYZ-breach-issue-projection`
- **Complete the local ticket schema (epic/parent/paired_with/blocked_on) (MBGQ89)** (in_progress, epic: ticket-anchor-external-bridge)
  Promote `epic`, `parent`, `paired_with`, and `blocked_on` from ad-hoc free-text frontmatter to canonical fields in the safeword ticket schema, validated by `safeword check` and enforced by the phase-advancement hook. (`depends_on` already landed via [AKZJXC](../AKZJXC-ticket-relations/ticket.md); this completes the remaining four and adds the phase-gating hook.)
  → `.project/tickets/MBGQ89`
- **Optional one-way coordination mirror to Linear/GitHub/Jira (consumes the ticket bridge) (THSPA5)** (in_progress, epic: ticket-anchor-external-bridge)
  ...
  blocked by: Ticket bridge — one-way provider-adapter layer (Linear/GitHub/Jira via Arcade MCP) (JS5K5G)
  → `.project/tickets/THSPA5-external-tracker-mirror`
- **Epic: Ticket system — local execution anchor + one-way bridge to external trackers (WG3Z2N)** (in_progress, epic: ticket-anchor-external-bridge)
  Settle one seam for safeword's ticket system — **local files are the canonical execution anchor; external trackers (Linear/GitHub/Jira) are one-way projections** via a shared provider bridge — and group the tickets that build each half so their couplings stay visible.
  → `.project/tickets/WG3Z2N-ticket-anchor-external-bridge`
...
- **Let Codex verify task tickets without Claude session proof (HMZSCD)** (in_progress, epic: —)
...
- **Let Codex earn self-review stamps without render-time shell (K2ZP40)** (in_progress, epic: —)
...
- **Model-tier policy: where to use opus / sonnet / fable across safeword tasks (Z4Q24Q)** (in_progress, epic: —)
  Decide which safeword tasks/skills/sub-agents run on which model tier — fable (frontier: hardest/longest autonomous work), opus (strong general reasoning), sonnet (fast/cheap, judgment-light) — so capability lands where it pays and cost is saved where it doesn't.
