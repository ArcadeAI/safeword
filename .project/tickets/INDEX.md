...
- **Task: ESLint 10 Migration (099)** (in_progress, epic: —)
...
- **Let users choose where to require human review vs. full autonomy (90AZDV)** (in_progress, epic: —)
  Let a team set, per kind of decision, where safeword stops for human review and where it runs autonomously — and when autonomous, resolve the breakpoint with a context-loaded sub-agent calling `/figure-it-out` instead of pausing.
  → `.project/tickets/90AZDV-configurable-hitl-autonomy`
...
- **Align ticket system docs with real folder shape (BHR7DK)** (in_progress, epic: —)
  Make the ticketing instructions describe the real `safeword ticket new` folder shape and supported historical formats.
  → `.project/tickets/BHR7DK-align-ticket-system-docs-with-real-folder-shape`
...
- **YOLO mode: route user-feedback pauses through /figure-it-out (G2E72G)** (superseded, epic: —)
...
- **Set an autonomy posture and let the agent resolve trusted decisions on its own (HPQ43R)** (in_progress, epic: —)
  Ship the v1 spine of configurable HITL — a project/personal autonomy policy set by preset, with autonomous axes resolved by a context-loaded sub-agent and a fail-safe defer when it can't decide — leaving the control-ladder tiers to follow-up children.
  → `.project/tickets/HPQ43R-autonomy-posture-spine`
