...
- **Reliable two-tier review-spec eval (corpus + GEPA re-run + harness gate) (21RAT9)** (done, epic: —)
  Make the review-spec behavioral eval reliable enough to adjudicate a prompt change: expand and de-noise the fixture corpus into GEPA feedback/pareto/test splits, re-run GEPA on the bare-model proxy with the corpus-structure leak stripped and a verbosity guard, and add a Tier-2 headless-harness validation runner (claude -p / codex exec) as a hard ship gate.
  blocked by: GEPA prompt optimization for the review-spec skill (E2D8S5)
  blocks: Adopt the lean review-spec candidate into the live skill (M3SI7T), Generalize the eval discipline for any skill — playbook + reference impl, not a premature framework (XZFSZ5)
  → `.project/tickets/21RAT9-review-spec-eval-hardening`
- **Adopt the lean review-spec candidate into the live skill (M3SI7T)** (in_progress, epic: —)
  Ship the lean candidate — validated at Tier-1 (−46% false alarms) and Tier-2 (−34%, floor clean, real `claude -p` harness), recall intact — into the production review-spec skill across all harnesses.
  blocked by: Reliable two-tier review-spec eval (corpus + GEPA re-run + harness gate) (21RAT9)
  → `.project/tickets/M3SI7T-adopt-lean-review-spec`
- **Generalize the eval discipline for any skill — playbook + reference impl, not a premature framework (XZFSZ5)** (done, epic: —)
  Make the eval-driven-skill-optimization method reusable so any skill's prompt changes can be adjudicated by evidence — by capturing the METHODOLOGY (a playbook) and a copyable REFERENCE IMPLEMENTATION (review-spec's experiment), not by building a shared code framework two divergent evals would both fight.
  blocked by: Reliable two-tier review-spec eval (corpus + GEPA re-run + harness gate) (21RAT9)
  → `.project/tickets/XZFSZ5-generalize-skill-eval-harness`
...
- **Choose local or remote test execution per contributor (S7TZF9)** (done, epic: —)
...
### eval-driven-skill-optimization

- **Eval-driven skill optimization — a reusable harness to measurably improve any skill prompt (7ZLTWB)** (in_progress, epic: eval-driven-skill-optimization)
  Turn the review-spec eval machinery (built in E2D8S5, hardened in 21RAT9) into a reusable, skill-agnostic harness — so a proposed prompt change to ANY skill is adjudicated by evidence, not taste, before it ships.
  → `.project/tickets/7ZLTWB-eval-driven-skill-optimization`
...
- **Environment-portable tracker transport (plan + pluggable executor) (CBTDK8)** (done, epic: offboard-local-ticketing)
...
- **Resume interrupted closeout after a Codex upgrade (07VEZF)** (in_progress, epic: —)
  Carry exact pending closeout targets safely into the next protected Codex task.
  → `.project/tickets/07VEZF-resume-closeout-after-upgrade`
- **Use complete hosted CI evidence during closeout (0RDXVD)** (in_progress, epic: —)
  Skip redundant local verification only when exact hosted evidence covers the clean delivery head.
  → `.project/tickets/0RDXVD-trust-green-hosted-ci`
- **Keep closeout retrospective bounded to meaningful session work (CFK8P4)** (in_progress, epic: —)
  Let Codex closeout ignore only its positively attributed bookkeeping while preserving fail-closed review of genuine additions.
  external issue: https://github.com/ArcadeAI/safeword/issues/3337
  → `.project/tickets/CFK8P4-bound-closeout-retro`
...
- **pr-review-eval (CWGYH0)** (in_progress, epic: —)
  ...
  external issue: https://github.com/ArcadeAI/safeword/issues/1910
...
- **Report uncertain Codex protection honestly (03Q0GR)** (done, epic: —)
  Prevent Codex upgrades from falsely telling customers that Safeword is wholly inactive.
  external issue: https://github.com/ArcadeAI/safeword/issues/2806
  → `.project/tickets/03Q0GR-truthful-codex-status`
...
- **Make large behavior specifications navigable for maintainers (05Z2TJ)** (done, epic: —)
  Split the offload-tests specification into cohesive Rule-aligned files without changing its behavior contract
  external issue: https://github.com/ArcadeAI/safeword/issues/2583
  external PRs: https://github.com/ArcadeAI/safeword/pull/2596
  → `.project/tickets/05Z2TJ-make-large-behavior-specifications-navigable`
...
- **Bring proven product patterns into every feature (0XZAYA)** (in_progress, epic: —)
  Make every feature capture product and implementation exemplars before committing to behavioral and technical decisions.
  → `.project/tickets/0XZAYA-learn-from-exceptional-products`
...
- **Honor host JavaScript toolchains during agent edits (13E3EN)** (done, epic: —)
  Let Safeword run the JavaScript toolchain a project has already chosen without losing Safeword quality gates.
  → `.project/tickets/13E3EN-honor-host-toolchains`
...
- **Keep lint-hook edits free of Safeword upgrades (1511)** (done, epic: —)
  external issue: https://github.com/ArcadeAI/safeword/issues/1511
  → `.project/tickets/1511-keep-lint-hook-off-network`
...
- **Keep quality reviews observable and actionable (1YYG74)** (done, epic: —)
  Let Safeword complete long independent reviews with visible bounded progress and precise recovery without corrupting machine output.
  → `.project/tickets/1YYG74-reliable-observable-quality-reviews`
...
- **Make each agent's plugin fully self-contained (2C1E82)** (in_progress, epic: —)
  Neither Claude nor Codex should depend on project-local .safeword/hooks, .safeword/skills, or .safeword/scripts once selected without the other — each plugin package ships and runs its own copies
  → `.project/tickets/2C1E82-self-contained-agent-plugins`
- **Keep agent-created pull requests in draft until approved (2CQCAK)** (done, epic: —)
  Make every agent-created pull request start as a draft unless the user explicitly asks for a ready pull request.
  external PRs: https://github.com/ArcadeAI/safeword/pull/2117
  → `.project/tickets/2CQCAK-keep-agent-created-prs-draft`
...
- **Track Codex plugin reload fixes for Safeword users (2HXHSV)** (in_progress, epic: —)
  Alert maintainers when the upstream Codex plugin-reload issues close so Safeword can reassess its restart guidance.
  external issue: https://github.com/openai/codex/issues/17636
  → `.project/tickets/2HXHSV-codex-plugin-reload-watch`
...
- **Let teams relocate the BDD lane without a stray default folder (2N1MQF)** (in_progress, epic: —)
  When paths.features/paths.steps are configured, stop scaffolding and scanning the unused default features/ and steps/ directories
  → `.project/tickets/2N1MQF-relocate-bdd-lane-cleanly`
- **Confirm active Codex protection after plugin updates (2Q5V78)** (done, epic: —)
  Make Codex hook activation produce trustworthy proof after a plugin update so users are not trapped in a false restart loop.
  → `.project/tickets/2Q5V78-restore-codex-hook-activation`
...
- **Create lean product plans for features and epics (2Y4B5R)** (done, epic: —)
  Have Safeword create decision-ready Product Plans without duplicated authoring or busy work
  → `.project/tickets/2Y4B5R-lean-product-plans`
...
- **Make every ready PR understandable and immediately mergeable (3579)** (done, epic: —)
  external issue: https://github.com/ArcadeAI/safeword/issues/3579
  → `.project/tickets/3579-reviewer-as-customer-pr-readiness`
...
- **Send cloud retros silently without approval prompts (3F5Z6P)** (in_progress, epic: —)
  Let SafeWord quietly submit a sanitized retrospective from local Claude Code and local Codex, and call it preserved only when the durable receipt arrives — without interrupting the user or requiring signup.
  → `.project/tickets/3F5Z6P-send-cloud-retros-silently`
- **Make independent review failures actionable (3FK4DC)** (done, epic: —)
  Make installed-reviewer discovery, failure diagnosis, and evidence packet roles reliable in Codex-hosted reviews.
  external issue: https://github.com/ArcadeAI/safeword/issues/2386
  → `.project/tickets/3FK4DC-reliable-independent-review`
- **Judge work against its authoring rubric (3HM021)** (in_progress, epic: —)
  external issue: https://github.com/ArcadeAI/safeword/issues/3119
  → `.project/tickets/3HM021-judge-work-against-its-authoring-rubric`
...
- **Keep Codex protection active after plugin updates (46BF8H)** (in_progress, epic: —)
  Ship every hook dependency required by the standalone Codex plugin runtime so installed hooks execute without project-local bootstrap files.
  → `.project/tickets/46BF8H-package-codex-hook-runtime`
...
- **plan-gh-parity: assert plan order against live order, not just membership (4PCMAE)** (done, epic: —)
...
- **Activate Safeword upgrades coherently in Codex (4S2S8V)** (done, epic: —)
...
- **Preview every install change before mutation (5JKNQG)** (done, epic: —)
  Make install planning enumerate the complete project effect set before apply.
  external issue: https://github.com/ArcadeAI/safeword/issues/2479
  → `.project/tickets/5JKNQG-preview-every-install-change`
...
- **Keep override regressions fast for maintainers (5KHSQB)** (done, epic: —)
  Cut override-survival test time without weakening any customer override example.
  external PRs: https://github.com/ArcadeAI/safeword/pull/1478
  → `.project/tickets/5KHSQB-keep-override-regressions-fast`
...
- **Deliver every eligible local retro finding in one bounded batch (758MH1)** (done, epic: —)
  Submit every eligible sanitized finding from one local session in a single bounded public request without changing recovery behavior.
  external issue: https://github.com/ArcadeAI/safeword/issues/3477
  → `.project/tickets/758MH1-deliver-local-retro-batches`
...
- **Keep default BDD lanes visible without manual configuration (8B4GVR)** (done, epic: —)
  ...
  → `.project/tickets/8B4GVR-root-bdd-lane-advisory`
...
- **Correct repeated Stop feedback without losing reply compliance (92NVVY)** (done, epic: —)
  Replace repeated full Claude Stop contracts with exact grammar-derived corrections while preserving first-rewrite compliance.
  external issue: https://github.com/ArcadeAI/safeword/issues/1758
  → `.project/tickets/92NVVY-correct-repeated-stop-feedback`
...
- **Keep audits focused on stacked branch work (AUDSTK)** (done, epic: —)
  → `.project/tickets/AUDSTK-audit-stacked-branch-scope`
...
- **Keep reviews focused on authored changes (CKWE2D)** (in_progress, epic: —)
  Let independent reviews automatically exclude explicitly generated oversized artifacts while reporting the reduced scope.
  external issue: https://github.com/ArcadeAI/safeword/issues/2121
  → `.project/tickets/CKWE2D-keep-reviews-focused-on-authored-inputs`
...
- **Keep routine retros running after tool-list changes (D7FWME)** (done, epic: —)
  Let routine retro submission proceed without unrelated manual dependency-fingerprint recovery.
  → `.project/tickets/D7FWME-prevent-stale-deps-blocking-retros`
...
- **Keep Cursor quality proof recording reliable (DDFA7X)** (done, epic: —)
  → `.project/tickets/DDFA7X-keep-cursor-skill-proof-recording`
...
- **Keep behavior tests reliable for contributors (ECGEP9)** (in_progress, epic: —)
  Make the executable BDD lane finish reliably and prove every no-mutation claim it advertises.
  external issue: https://github.com/ArcadeAI/safeword/issues/2582
  → `.project/tickets/ECGEP9-reliable-bdd-gates`
...
- **Turn offload specifications into trustworthy executable coverage (F12DMP)** (done, epic: —)
  Make offload BDD coverage honest, readable, and incrementally executable.
  external issue: https://github.com/ArcadeAI/safeword/issues/2624
  → `.project/tickets/F12DMP-executable-focused-offload-bdd`
- **Let test queues recover locks created by older runners (F137WW)** (done, epic: —)
  Safely recover abandoned package-test locks written by the immediately preceding runner format.
  → `.project/tickets/F137WW-recover-legacy-test-locks`
...
- **Let users rank local reviewers and models (FJAKRN)** (done, epic: —)
  Detect usable local review routes and follow a user-defined reviewer/model fallback order.
  → `.project/tickets/FJAKRN-rank-local-reviewers-and-models`
...
- **Keep independent review available through OpenCode (FZTWG0)** (done, epic: —)
  Use OpenCode as an independent review option and fallback without weakening review provenance or read-only guarantees.
  → `.project/tickets/FZTWG0-opencode-independent-review-fallback`
...
- **Keep Codex reviews reliable without package bootstrap (GVT6GE)** (done, epic: —)
  Run Codex Safeword review workflows from the installed plugin even when Bun package installation is corrupted or concurrent.
  → `.project/tickets/GVT6GE-reliable-codex-review-dispatch`
...
- **Keep package tests from deleting the CLI they exercise (JZ6PNV)** (done, epic: —)
  Keep the built CLI and preset artifacts stable for the full lifetime of every package test run.
  external issue: https://github.com/ArcadeAI/safeword/issues/1823
  → `.project/tickets/JZ6PNV-keep-built-cli-stable-during-tests`
...
- **Bound the Codex reference-path rewrite to real Markdown links (KEQQGN)** (done, epic: —)
...
- **Finish closeout reliably across verification and restarts (KMB053)** (in_progress, epic: —)
  Let closeout finish safely using exact evidence across CI, live transcripts, and required Codex restarts.
  → `.project/tickets/KMB053-resilient-closeout`
...
- **Keep observed Codex protection visible across upgrades (M35AP7)** (in_progress, epic: —)
  Preserve trustworthy task-bound proof of an older loaded Safeword runtime across profile upgrades while keeping current-version activation exact and fail-open.
  external issue: https://github.com/ArcadeAI/safeword/issues/2806
  → `.project/tickets/M35AP7-codex-upgrade-protection`
...
- **Let installed Codex hooks prove protection (P4A681)** (done, epic: —)
  Make the versioned Codex plugin runtime record lifecycle proof from its installed cache layout.
  → `.project/tickets/P4A681-let-installed-codex-hooks-prove-protection`
...
- **Prevent unfinished BDD features from closing (PD63KG)** (done, epic: —)
  Block ticket completion when its referenced feature still contains @wip, consistently across supported agent hosts including OpenCode.
  → `.project/tickets/PD63KG-prevent-wip-feature-completion`
...
- **Close Codex tickets when evidence passes (QRX2DN)** (done, epic: —)
  Let Codex complete verified tickets before a ready PR needs a manual done-flip commit.
  external issue: https://github.com/ArcadeAI/safeword/issues/1388
  → `.project/tickets/QRX2DN-codex-done-gate-auto-transition`
...
- **Judge implementation plans by their authoring standard (RMCJCV)** (in_progress, epic: —)
  Keep plan authoring and independent review aligned to one canonical judgment contract.
  → `.project/tickets/RMCJCV-judge-implementation-plans-by-their-authoring-standard`
...
- **Review the turns that edited files, even when tool output carries a trailing note (S0RYNS)** (in_progress, epic: —)
  Stop treating a tool-result message as the start of a human turn, so the quality review still fires on turns that changed files.
  → `.project/tickets/S0RYNS-review-turns-that-edited-files`
- **Keep Codex Desktop proof session-bound (S2CWBE)** (done, epic: —)
  Let Codex Desktop record quality proof when its PreToolUse cache bridge is unavailable.
  → `.project/tickets/S2CWBE-codex-desktop-session-proof-fallback`
...
- **Make closeout preview and apply converge for merge sessions (TFG4CR)** (done, epic: —)
  Allow the current merged session to preview and apply closeout safely even when transcript progress changes or binding support was installed during the session.
  → `.project/tickets/TFG4CR-closeout-preview-apply-convergence`
...
- **Keep project-local Ultracite child tools discoverable (ULTRBI)** (done, epic: —)
  Let Safeword run a host project's local Ultracite exactly as installed, including Ultracite's call to the sibling local Biome binary.
  → `.project/tickets/ULTRBI-project-local-ultracite-child-tool`
...
- **Retire the .agents/skills dogfood residue (V5V4YP)** (done, epic: —)
...
- **Review work with enough context to avoid false findings (V7TSC4)** (done, epic: —)
  Give independent quality reviewers the minimum supporting evidence needed to validate each target without reviewing or dumping unrelated files
  → `.project/tickets/V7TSC4-review-with-enough-context`
...
- **Configure review routes by scope (W8M4Q2)** (done, epic: —)
  Let users keep their preferred review models locally while projects retain explicit authority.
  → `.project/tickets/W8M4Q2-configure-review-routes-by-scope`
...
- **Keep stop-quality prompts scoped to edited-work turns (WSFBVS)** (done, epic: —)
  Skip decision-brief prompts for conversational follow-up turns while retaining the edited-work quality review.
  external issue: https://github.com/ArcadeAI/safeword/issues/1096
  → `.project/tickets/WSFBVS-scoped-stop-quality-review`
- **Keep focused verification responsive for developers (WZF6JF)** (in_progress, epic: —)
  Give a developer a prompt, actionable outcome when a focused Vitest run is blocked by another worktree, without ever running a second Vitest process.
  external issue: https://github.com/ArcadeAI/safeword/issues/1484
  → `.project/tickets/WZF6JF-keep-focused-verification-responsive`
...
- **Attach useful runtime context to retros without signup (XGQQT2)** (done, epic: —)
  Give maintainers enough privacy-bounded runtime context to understand retro findings while keeping capture silent and registration-free.
  external issue: https://github.com/ArcadeAI/safeword/issues/3429
  → `.project/tickets/XGQQT2-retro-runtime-context`
...
- **Keep review workflows runnable without a global Safeword binary (XTY8FE)** (done, epic: —)
  Make every Safeword review workflow resolve a compatible CLI in installed projects and source worktrees.
  external issue: https://github.com/ArcadeAI/safeword/issues/2185
  → `.project/tickets/XTY8FE-resilient-review-cli-resolution`
- **Run GitHub live smokes without waiting for builds (Y1484A)** (done, epic: —)
  Let maintainers run the two proven source-only GitHub smokes while an unrelated package test owns the normal build-and-Vitest lock.
  → `.project/tickets/Y1484A-run-github-live-smokes-without-waiting-for-builds`
- **Prove cross-provider review before scaling spend (Y4ZAAY)** (in_progress, epic: —)
  Run a bounded development-only canary over the legacy-provenance corpus using only OpenAI GPT-5.6 Terra for review, with durable attempt and cost stops.
...
- **Bind spec invariants to falsifying scenarios (Y9P3ZC)** (done, epic: —)
...
- **Keep verification bound to the current work (Z24K1J)** (in_progress, epic: —)
  Ensure verify resolves only the ticket relevant to the current PR or worktree context.
  external issue: https://github.com/ArcadeAI/safeword/issues/2083
  → `.project/tickets/Z24K1J-keep-verification-bound-to-current-work`
...
- **Give OpenCode builders full Safeword protection (ZM38A2)** (done, epic: —)
  Make Safeword workflows and automatic guardrails work through OpenCode with truthful cross-host parity.
  → `.project/tickets/ZM38A2-opencode-parity`
