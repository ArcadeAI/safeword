...
| Reuse existing durability and deadline | Keep project UUID, session-scope claim, SQLite raw-body store, preparation deadline, one bounded public handoff timer, and recovery lane | Add a metadata store, queue, retry, or background worker | The existing path already supplies identity, durability, dedupe, and nonblocking recovery |
