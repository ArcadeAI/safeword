...
| Each installed host review stage receives relevant configured knowledge | Wiring integration: real config → setup/generated host artifact → configured resolver → captured review input | Proves all 12 host×stage inputs with real collaborators, mocking only the temporary filesystem boundary; the captured input is the observable record. |
...
3. **Review-source production and host delivery.** Update canonical
   ...
   wiring matrix from the production sources—`schema.ts` Claude assets,
   `cursor-wrappers.ts`, and Codex `inventory.ts`/catalogue—then exercise it in
   `tests/integration/project-knowledge-review-entrypoints.test.ts`, capturing
   the resolved review input from each installed entry point in temporary
   projects.
...
| Host proof | Derive installed entry points from the production schema/Cursor/Codex catalogues, capture their resolved review inputs, and require independent review records for semantic outcomes [4] | Duplicate a test-only host matrix; string-search only; claim live-agent judgment from static tests | A duplicate matrix drifts, string search misses install wiring, and static files cannot prove judgment. |
