...
out_of_scope:
  ...
  - Arbitrary commands that bypass the package-test wrapper, deliberately detached POSIX descendants, and cross-user or cross-container serialization of one shared checkout.
...
done_when:
  ...
  - Before leaving scenario-gate, implementation removes feature-level `@wip`, makes every required platform job fail on zero selected scenarios, and records RED, GREEN, and REFACTOR evidence for every ledger entry.
