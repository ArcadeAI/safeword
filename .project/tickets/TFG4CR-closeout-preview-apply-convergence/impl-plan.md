...
1. Extend the retrospective receipt gate first. Each extraction window reads the transcript once, chooses the last complete newline-terminated JSONL record at or before observed EOF, and derives both offsets from that same immutable prefix: `byteLength` is its buffer length and `utf16Length` is `prefix.toString('utf8').length` for `String.prototype.slice`. The receipt digest covers exactly those `byteLength` bytes. An unchanged prefix is reusable; append-only growth validates that prefix, then closeout internally supplies the receipt-derived `utf16Length` to the existing public `retro --window-start` option. If extraction itself advances the transcript, closeout evaluates another immutable window in the same invocation, with a fixed three-window ceiling; continuous growth exhausts the ceiling and blocks cleanup. A partial tail remains outside the seal for the next invocation. Mutation, truncation, wrong provenance, missing evidence, pending filing, or failed delta retro remains blocking. Findings and acknowledgements persist before the receipt advances; a failed retro never advances it. Receipt replacement remains atomic and refuses to move an offset backward.
...
| Reuse bounded retrospective evidence | Seal each last-complete JSONL prefix from one read, validate its byte-exact predecessor, and extract from the receipt-derived UTF-16 offset; catch up for at most three immutable windows per invocation | Re-extract the whole transcript; wait for a future invocation after every append; retry without a ceiling; ignore append-only growth; accept size-only growth; let closeout accept a caller-provided offset | Whole-transcript extraction and future-invocation-only catchup recreate the loop; unbounded retries may never return; ignoring growth misses new work; size-only checks miss mutation; caller-provided closeout offsets bypass host provenance. The existing retro CLI offset remains public, but closeout never trusts an external value. |
