...
- 2026-06-16T18:55:00.000Z Started #2 (live wiring) — instruction layer: added an "Autonomy posture" section to SAFEWORD.md (template + installed, parity verified identical). It tells the agent to, at a breakpoint, map the decision to its axis, run `safeword autonomy show`, then ask (pause) or resolve-via-subagent+`/figure-it-out`-and-log, with the fail-safe defer and the always-confirm denylist/hard-gates. This is the documented behavioral driver for a live session.
- 2026-06-16T19:50:00.000Z Polish + infra (post-quality-review): (1) `autonomy show` now names its source — header reads `Autonomy posture — <preset> [+ personal overrides]` (kept in the header, not a keyValue line, so the BDD posture parser still sees only the 5 axis rows). 9 command tests green, config BDD lane still 12 green. (2) Raised the dev/CI flow to latest Node 22 via `/figure-it-out`: added `.nvmrc` (22.22.3, latest 22.x), root `engines: node >=22.12` (matches cli; not over-restrictive), and pointed ci.yml's test job at `.nvmrc` (single source). Kept the engines floor at 22.12 (commander 15's ESM minimum) rather than the latest patch. (3) Filed follow-up BRZ55C for the #2 hook + `@live` proof.
- 2026-06-16T19:10:00.000Z #2 sequencing finding: studied tests/smoke/steering.live.test.ts — its live assertion works because it checks a DETERMINISTIC hook signal (permission_denials), not model choice. Our v1 wiring is SAFEWORD.md prose (the least-reliable enforcement tier per the instruction-attention-hierarchy learning), so a `@live` test of prose-only wiring would be flaky AND can't run here (no ANTHROPIC_API_KEY/claude). Conclusion: the per-turn posture hook is the PREREQUISITE for a non-flaky live proof — it produces the deterministic signal the live test asserts on. Did not fabricate an unrunnable flaky test. REMAINING for #2 (recommended next slice): (1) a UserPromptSubmit/PreToolUse hook that surfaces the resolved posture and enforces the always-confirm denylist under autonomy (deterministic, unit-testable here; needs settings.json + schema + guide wiring); (2) a `@live` smoke test asserting on that hook's signal, run where creds exist.
