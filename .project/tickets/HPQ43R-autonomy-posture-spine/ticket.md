...
- 2026-06-16T18:55:00.000Z Started #2 (live wiring) — instruction layer: added an "Autonomy posture" section to SAFEWORD.md (template + installed, parity verified identical). It tells the agent to, at a breakpoint, map the decision to its axis, run `safeword autonomy show`, then ask (pause) or resolve-via-subagent+`/figure-it-out`-and-log, with the fail-safe defer and the always-confirm denylist/hard-gates. This is the documented behavioral driver for a live session. REMAINING for #2: (1) a `@live` smoke test (like tests/smoke/steering.live.test.ts) that drives a real agent under a Hands-off policy and asserts it resolved-vs-paused — written to run only where `claude` + ANTHROPIC_API_KEY exist (this sandbox lacks them); (2) optional reliability upgrade — a per-turn prompt hook that injects the resolved posture (instruction-attention-hierarchy: a hook steers more reliably than cross-file prose). These close the loop to /verify.
