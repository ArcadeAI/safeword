...
| `buildExtractArgv({model, systemPrompt, prompt, allowedTools})` | **Done** for execution | `allowedTools` param added (2026-07-21); `spawn.ts` grants `Read,Grep,Glob,Bash,WebSearch,WebFetch` in the execute tier and read-only on a fork, so R13/R17 run for real. **Still open:** the Claude path has no arcade-MCP wiring for R6 — only the Codex path passes `mcpServers`. A Claude-vendor review cannot yet reach the tracker. |
