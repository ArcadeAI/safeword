...
- 2026-06-21T15:30:00Z Ran /quality-review on the implemented change (fresh independent reviewer) + refactor pass. Reviewer flagged 2 CRITICALs: (1) regression — dropped the upstream "prefer the plain word" rule; (2) scannability — 180-word paragraph, 3× its neighbors, violating the section's own "write to be scanned" ethos. Fixed both by splitting into two tight single-idea rules: "Speak plainly" (restores plain-word preference) + "Gloss jargon at the decision point" (the NTB thread). Also applied reviewer SUGGESTs: "a step they must take" (was ambiguous "next step you need from them"), "Leave background narration unglossed" (was inverted phrasing), "Don't assume the user reads code" (was over-strong "can read code"). Both copies byte-identical.
