...
| Config robustness               | no `.safeword/config.json`; unparseable JSON; empty-string / non-string value; relative vs absolute path       | TB1.AC1/AC2 |
...
| Mention matching                | full name; scoped name matched by tail (`@scope/pkg` → `pkg`); case-insensitive; word-boundary (`cli` ≠ `click`) | TB2.AC1/AC2 |
