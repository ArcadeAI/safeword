...
- **Still a repository follow-up — the mandatory skill-authoring guide is absent:** `AGENTS.md` requires `.safeword/guides/skill-authoring-guide.md` for skill changes, but that file is not installed in this checkout. No product skill change was retained in this pass, so the missing guide did not block the epic, but the documented process currently points to an unavailable authority.
