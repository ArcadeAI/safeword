...
3. **Selected project authority (NTB1.R1).** Inspect all schema path buckets for native-only and Cursor-mixed selection. Use the real project reconciler with profile-manager boundaries substituted to prove removing one host preserves remaining Cursor/native hosts and authored project content, and repeated reconciliation does not restore native runtime. Observe unselected native profiles through existing adapters before removal; retain shared enrollment/configuration/dependencies and include these observations in the plan digest, without applying any unselected profile effects.
...
| Reject native project-runtime paths directly | Native workflow catalogues reject unavailable project-runtime paths regardless of runner, quoting, or Markdown shape | Grow the interpreter allowlist and parse every command occurrence | Six executable counterexamples passed the heuristic. Only one non-executable example needed host-neutral wording, so removing the heuristic is smaller and more complete. |
...
The final guard decision was checked against the actual generated catalogues, six failing command-shape regressions, the local Bash `help command` contract, and [CommonMark code-span rules](https://spec.commonmark.org/0.31.2/#code-spans) on 2026-09-02. A fuller parser could preserve literal legacy-path examples, but they provide no necessary native workflow capability. Premortem: a future migration explanation may need such a literal; keep migration documentation outside executable native catalogues or use host-neutral wording rather than weakening the guard.
