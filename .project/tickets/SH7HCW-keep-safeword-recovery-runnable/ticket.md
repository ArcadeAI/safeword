...
out_of_scope:
  ...
  - Changing Safeword behavior through non-`bunx` package executors.
done_when:
...
- 2026-08-07T05:25:00Z Post-fix scenario re-review approved cross-agent. Pinned recovery-only `&&` composition as allowed, documented the CLI-option whitelist maintenance trigger, clarified which rejection rows exercise the metacharacter guard versus the shared segment parser, isolated benign trailing arguments from shell-evaluation tokens, and added a scoped package lookalike boundary.
- 2026-08-07T04:05:00Z Final review hardening: covered environment-value substitution and reverse command ordering, added mutation evidence for substitution and segment splitting, made the Bun-only boundary explicit, rejected a consumed `--cwd` recovery verb, and made the metacharacter guard quote-aware so literal quoted paths remain usable while executable substitutions stay blocked. Final focused contract passes 157/157 tests and 36/36 scenarios.
