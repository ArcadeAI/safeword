...
- **NEW (2026-07-16, from the first real external reaction): are there TWO products here, and is the second one ours?** The architect's response to the posted goroutine leak: *"not actually a finding on the PR… I would consider it noise and ignore it. If it were a background agent doing nightly sweeps through our codebase looking for bugs, like Pascal's security agent does, then that's different."* A true, verified finding was **noise in the PR channel** because it wasn't about the change. This forks the design: (1) **PR reviewer** — scoped to the diff, off-topic findings are noise (§3.5 gate added to the skill); (2) **codebase sweep** — nightly, whole-repo, severity-ranked, where a latent 18-month-old bug is exactly on-topic. They share almost all machinery (the skill, the gates, cross-model, the verdict) and differ in corpus + triage model. Arcade already runs a security sweep (Pascal's), so the question is whether safeword *builds* the sweep, *feeds* the existing one, or stays PR-only and routes off-topic findings out. **Needs a product decision before G5337S is built — the §3.5 gate depends on where routed findings go.** Do not create the sweep ticket unilaterally.
