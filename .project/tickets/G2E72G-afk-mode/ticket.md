...
phase: intake
status: in_progress
external_issue: https://github.com/ArcadeAI/safeword/issues/3697
...
scope:
  ...
  - "M1 - independent-review precondition: /afk refuses to activate unless the review coordinator reports a genuine cross-agent route, and an in-flight AFK run halts if that route is lost (auth expiry, reviewer unavailable) rather than continuing on a degraded fallback"
...
done_when:
  ...
  - /afk refuses to activate when no independent review route is available, and an AFK run that loses its independent reviewer mid-flight halts instead of degrading to same-agent or self-review
...
- 2026-09-05T03:52:00.000Z Tracker synced. Three retro-filed issues described this same drift with three different canonical hashes (dedup missed): #3697 (2026-09-04 04:19), #3714 and #3719 (both filed by this session's own retro). Promoted **#3697** to the feature tracking issue — original finding retained, resolution appended (rename, routing design, load-bearing decisions, milestones, testing posture) — and closed #3714/#3719 as duplicates of it. Linked as `external_issue`. #3697 carried demand evidence the spec lacked: a real builder hit this gap and improvised "I told it to stop asking me for yes", i.e. reinvented an ad-hoc untested version of the mode the skill claimed existed. Folded into Product Bet / Why now.
- 2026-09-05T00:34:00.000Z **alex redirected the M1 precondition**: drop the proposed production-credential preflight, require a working independent reviewer instead. Added TBU3.R4 (was R4 → now R5). Rationale: an attended session has the builder as reviewer of last resort, so the coordinator's degradation ladder (independent → same-agent headless → host fresh-context → bounded self-review) is safe to walk down. AFK removes that backstop, making the fallback rungs wrong exactly when reached. Coordinator already distinguishes `independence: 'cross-agent'` from same-agent and emits "No independent check was recorded", so the precondition reads existing state rather than inventing a signal. Must be continuous, not just preflight — auth can expire mid-run (cf. #3612). Capability reduction (credential scoping, sandboxing) recorded as an M1 non-goal: Safeword is a process layer installed into the builder's project and does not own the runtime that would enforce it; the denylist is explicitly not provably complete.
