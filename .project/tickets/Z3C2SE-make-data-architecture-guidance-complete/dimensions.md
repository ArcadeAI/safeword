...
| Delivery surface | canonical template; CLI-owned `.safeword` installation read by Cursor/Codex; path-adapted Claude plugin resource; Codex planning route with zero additional guide copies; OpenCode catalogue with no guide copy or delivery-specific planning path | SWM1.R1 |
