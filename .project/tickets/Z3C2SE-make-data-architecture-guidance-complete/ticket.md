...
done_when:
  ...
  - a documented cold-start command uses the adapter's fixed supported inference controls and records model/version, decoding configuration, exact prompt, canonical guide hash, case/rubric digest, and response for all nine cases; the recorder and deterministic verifier re-grade against the current rubric and reject stale hashes, missing records, a recorded model/version or decoding configuration that differs from the checked-in evaluation contract, or a response that fails its rubric
  ...
  - mutable values in authored evaluation case prose use a fixed synthetic-placeholder convention or checked-in migration-equivalent inputs; a deterministic scan of that prose rejects credential/token/key prefixes, email-shaped strings, and non-placeholder high-entropy values, and tests seed each rejection class, so no production credentials, secrets, real ciphertexts/nonces/tokens, or plaintext customer data are needed or stored
  ...
  - a repository check confirms the OpenCode profile catalogue contains no data-architecture guide copy or delivery-specific planning path, documenting why that surface remains unaffected while allowing shared workflow prose to name the guide
...
phase_anchors:
  ...
  - "verify: .project/tickets/Z3C2SE-make-data-architecture-guidance-complete/test-definitions.md"
  - "done: .project/tickets/Z3C2SE-make-data-architecture-guidance-complete/verify.md"
external_issue: https://github.com/ArcadeAI/safeword/issues/4560
created: 2026-09-13T22:52:33.607Z
last_modified: 2026-09-22T06:18:49.000Z
---
...
- 2026-09-22T06:18:49.000Z Verify: full configured verification, the 1503-scenario acceptance lane, lint, typecheck, builds, diff-scoped audit, and cross-agent Claude Opus review passed. Reconciled current `origin/main`, applied the review's delivery and corpus hardening, and bounded the remaining ablation-strength caveat.
