...
- 2026-06-16T17:10:00Z Audit: Full skill+guide audit complete across all 24 skills and 9 guides. Child ticket FJKM4X created for the 4 confirmed edits (planning-guide, design-doc-guide, data-architecture-guide, debug/SKILL.md). Remaining candidates resolved via /figure-it-out — verdicts below:
  - `SAFEWORD.md` Replan section: **Edit needed** (separate from FJKM4X; ZBVGPF priority item). Post-verdict main-agent behavior for change-scope/split has no bridge to figure-it-out. Fix: add trigger after "proposing one of still-good / change-scope / cancel / split / merge, with rationale" — "If the verdict is change-scope or split, run `/figure-it-out` before proposing a new approach." Rationale: sub-agent is report-only; figure-it-out must run in main context; only fires on re-decide verdicts (still-good/cancel need no research).
