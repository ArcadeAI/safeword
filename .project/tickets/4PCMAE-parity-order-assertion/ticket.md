...
- 2026-07-26T04:26:00.000Z Re-opened verify: the first close ran the commands by hand rather than invoking /verify, and skipped /audit on my own reasoning that a test-only diff had nothing to inspect. /audit then found FOUR defects in the changed file — a duplicate BLOCKED1 ticket id (residue from the abandoned mutant-E attempts; a repeated id makes the map lookup and any graph edge naming it ambiguous), a doubled toBeDefined assertion, a twice-written comment, and a docstring attached to the wrong constant. Fixed in 46687c2. Removing the duplicate forced a re-proof of ALL five mutants (it sat in the shared corpus, so it could have propped up any of them) — all five still fail. Also corrected a number I had been repeating: tracker-sync is 167 green across 22 files, not 138. Final tree: 5458 pass / 7 skipped / 369 files, 2 excluded failures; gherkin 497 scenarios (494 pass, 3 skipped); build + tsc clean.
