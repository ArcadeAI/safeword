...
- **Context pricing tier:** Each provider turn selects its own tier from retained total `input_tokens`, calculated as uncached plus cached plus cache-write tokens: short-context through 272,000 and long-context above 272,000. Frozen standard rates per million tokens are short-context `$2.00` uncached input, `$0.20` cached input, `$2.50` cache-write input, and `$12.00` total output; long-context rates are `$4.00`, `$0.40`, `$5.00`, and `$18.00` respectively. Retained `output_tokens` already includes reasoning tokens and is charged once. Attempt cost sums every retained turn cost exactly once. Cache-write usage is read only from the OpenAI-native detailed-usage field; an absent field normalizes to zero, while Anthropic-shaped usage makes the route invalid.
...
- **Fixture authorization:** The live authorization guard is categorically inapplicable to non-authorization fixture scenarios because they cannot dispatch. It is evaluated only by the live paid canary and the two scenarios explicitly named for missing authorization.
