...
- **Observed cost stop:** A post-attempt stop, not an in-flight provider quota. The completed attempt that crosses the amount is retained and reported; the next attempt is blocked. The authorization therefore blocks further attempts at $15 but accepts exposure to the cost of the one already-started attempt, whose work is separately bounded by the fixed verification, tool-call, and wall-clock policy.
...
- **Decision reason:** `eligible` only when every applicable pre-call accounting and authorization guard passes; otherwise the complete deterministic set of `attempt-stop`, `cost-stop`, `incomplete-attempt-accounting`, `incomplete-cost-accounting`, and `missing-authorization` reasons is reported. Every feature phrase `with reason`, `with reasons`, or `reports` means that exact exhaustive set. A limit's stop reason is evaluated only when that limit's durable accounting is complete; incomplete accounting reports its own reason and fails closed. A paid request without prior durable intent is incomplete attempt accounting. Durable intent followed by a provider request without a durable completion/cost record is incomplete cost accounting. Exclusive-lock contention is a separate pre-decision operational rejection (`canary is already running`), not a decision reason.
- **Route defect:** Route-invalid evidence reports the complete deterministic set drawn from `provider-mismatch`, `model-mismatch`, `service-tier-mismatch`, `non-native-usage`, `missing-response`, `malformed-response`, `duplicate-response`, `turn-mismatch`, `missing-review-stage`, and `missing-paid-turns`. The feature's `with exactly` phrasing means that exhaustive set; fixtures isolate one defect unless a scenario explicitly composes them.
...
- **Live authorization:** The explicit authorization is not a local boolean. It is the unique exact unedited allowlisted-maintainer upstream authorization for one output identity plus its unique initialization, per-attempt start, and per-attempt completion receipts and matching immutable local marker, all bound to the canonical repository, recomputed corpus digest and registration, annotated adapter and harness tags and commits, GPT-5.6 Terra/default route, ten-attempt cap, and $15 stop. Each isolated code checkout may remain off-main only when its canonical remote advertises the exact annotated tag object and peeled authorized commit. The authorization—not either tag—is the authority; the registration commit must be an ancestor of the exact authorized harness tag and must agree byte-for-byte with its earlier unedited anchor. Abandoned authorizations and receipts for other bindings remain visible for audit but cannot authorize or corrupt this identity. Missing, edited, duplicate-exact, stale, replayed-to-another-output or repository, or mismatched evidence reports `missing-authorization` and cannot dispatch. The authorization permits at most one initialization receipt and ten start/completion receipt pairs for that output identity.
...
- **Paid canary execution:** `@paid-canary` is excluded from default and CI test selection. It runs only after explicit maintainer authorization from clean checkouts of the authorized harness and adapter commits, with SDK/HTTP automatic retries disabled and the per-attempt policy fixed at 25 verifications, 40 tool calls per expert, and 360 seconds per expert. Canonical remote checks finish before the durable attempt starts; local checkout and input/context integrity are rechecked immediately before the start receipt and again before dispatch. It records and verifies the upstream start receipt and local intent before its first request, and any retry is a new controller-owned attempt. It cannot be rerun once the durable attempt or cost guards block it. Every non-`@paid-canary` scenario uses isolated fixtures, issues no provider request, and cannot mutate the real canary ledger; authorization-guard scenarios may evaluate that pure pre-dispatch guard without dispatching.
