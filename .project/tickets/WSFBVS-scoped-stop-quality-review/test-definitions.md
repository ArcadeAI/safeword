...
| Task notification starts a turn | User prompt A → assistant `Edit` → user `tool_result` → assistant completion A → user record containing only a complete `<task-notification>` block → assistant text-only status completion B → stop | Exit 0; stdout is empty. The notification starts turn B, whose assistant response made no edits. |
