...
- `uv run --locked mypy .` uses the repository lock and passes.
...
- Resolve the real typecheck plan and prove it selects `uv run --locked mypy .`.
- Run `uv sync --locked` and `uv run --locked mypy .`.
...
- 2026-09-11 Verified: `mise exec -- uv sync --locked` and `mise exec -- uv run --locked mypy .` pass; focused contract and resolver tests pass.
