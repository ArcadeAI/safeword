...
0. **Prove the lifecycle seam first.** Own and finish the dispatcher seam here.
   ...
   that session three numbered launch slots; every other session has one slot.
...
| Hook cadence | In-process plugin-mode/attention fast paths plus a cooperative 2,000 ms deadline; at most three automatic calls in the first session and one per later session; deterministic fake-clock tests and five cold disposable-fixture RC runs capped at 1,500 ms | Unlimited calls; subprocess timeout; never attempt automatically; hard shared-CI wall-clock threshold | Host responsiveness and attempt bounds cover deadlines, stable conflicts, and pre-mutation failures. Observation or catalogue changes re-arm work; cold release-host evidence protects the prompt-one expectation without making shared-runner speed a flaky shared-CI gate |
