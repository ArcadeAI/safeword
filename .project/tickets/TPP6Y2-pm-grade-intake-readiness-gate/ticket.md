...
- The target is a _sufficiency threshold_ — knowing when you know enough — not maximal questioning. Both over-asking and under-asking are documented failure modes ([CaRT](https://arxiv.org/pdf/2510.08517); [Learning to Ask](https://aclanthology.org/2025.emnlp-main.1104.pdf)).
...
**Honest delta (not greenfield):** `prompt-questions.ts` already emits an `intake` reminder with a three-part self-test ("what changes, what stays the same, observable done state"), mirroring the specificity self-test in `SAFEWORD.md`. This ticket is a _targeted expansion_ of that existing line, plus one VoI sentence in SAFEWORD.md — not a new gate.

- **Surfacing (resolved): compressed pointer in the prompt hook + detailed prose in SAFEWORD.md** — the established channel rule from `instruction-attention-hierarchy.md` (hook reminder ~95% and the only channel that fires when work is scoped without a skill; SAFEWORD.md-only is mid-file ~50% / cross-file ~20% — i.e. the current dormancy). The hook carries a terse keyword pointer (≤~20 words, e.g. `intent · done · what-must-not-break · riskiest-assumption+cheapest-test · problem-or-guess?`); SAFEWORD.md carries the generative phrasing. Not either/or.
- Expand the existing intake self-test from three dimensions to **five**, derived from four independent intake disciplines (see Research). The reminder stays a **compressed pointer** to the dimensions (as today's line is), NOT five spelled-out prompts re-injected every turn — that would itself become the per-turn interrogation this ticket avoids. The five dimensions:
  ...
  5. **Request shape** — is this the problem, or someone's guess at the fix? Normalize by requester (personas, now formalized as TB/NTB on main): a **Non-Technical Builder (NTB)** tends to give under-specified, domain-language asks — fill the constraints they won't state; a **Technical Builder (TB)** tends to hand a solution-in-disguise — recover the underlying problem before acting.
- **Fire in the pre-ticket / early-Clarify window, not only once an intake-phase ticket exists.** Today's intake reminder is inside the active-ticket branch of `prompt-questions.ts`; the motivating first-turn "go run" case is the `No active ticket` branch. The pointer must attach there (pre-classify) too — otherwise it stays dormant for exactly the case that motivated the ticket.
- Reuse the shipped `## Open Questions` section in `spec.md` (ticket V6N5PW) as the ledger of unresolved unknowns.
- Codify the VoI triage rule in SAFEWORD.md prose: reversible/local → run; irreversible/high-blast → resolve unknowns first. (The _behavioral go/no-go_ — ready when remaining questions are edge-cases not basics, `doc-coauthoring/SKILL.md:97` — is the quality bar for this prose, not a separately testable criterion.)
- **Reuse `elicit`, don't duplicate it.** The gate is the go/no-go threshold + the five dimensions; it _inherits_ elicit's rules — the Iron Law (`elicit/SKILL.md:11`, never surface a question the agent could answer itself), information-gain ordering (`:28`), the anchoring guard (`:50`), and the stopping rule (`:54`) — and _escalates to_ `/elicit` only when user-only unknowns remain at meaningful blast radius. The default path is a silent "go." The gate never re-implements question-asking.
...
- **Never announce the framework.** Render the five prompts as plain questions, not "running the readiness assessment" (brainstorm anti-pattern, `brainstorm/SKILL.md:49`) — already house law, reused not re-derived. **Persona-grounded:** the NTB persona judges by whether it works/is safe and treats internal jargon as "a dead end" (`personas.md:20`), so plain-language is a correctness requirement here, not just style.
- **Behavioral go/no-go phrasing** (from `doc-coauthoring/SKILL.md:97`): phrase the SAFEWORD.md prose so "ready" means _remaining questions are edge-cases/trade-offs, not basics_ — the checkable form of the signal and the anti-rubber-stamp guard. This shapes the prose copy; it is not a separate hook-testable criterion.
...
- **Leadership / mission command** → clarify _intent + end-state_, take prudent risk within it; depth scales with readiness × blast radius. (`intake-readiness-leadership-analogy.md`)
- **Product management / Cagan** → bounded core (objective, success-signal, problem-not-solution, who) + _riskiest assumption_; lightweight, not a brief. (`intake-readiness-pm-analogy.md`)
- **Architecture** → _constraints / what-must-not-break_ as the expensive-to-retrofit axis, made measurable. Bounded hard by **constraint decay** — agents degrade as constraints pile up, so surface only the load-bearing one. ([arXiv 2605.06445](https://arxiv.org/abs/2605.06445))
- **Consulting** → _problem-behind-the-problem_ / request-shape normalization (under-specified non-technical ask vs. solution-in-disguise technical ask). (`intake-readiness-architect-consultant-analogy.md`) Main's persona split (#301) independently formalized this same axis as TB/NTB — corroboration, now referenced by name in the request-shape dimension.

## Implementation notes (post main-merge, 2026-06-21)

main merged ESLint 10 + `eslint-plugin-unicorn` 68 (cli package only; root stays on eslint 9) and a tsconfig `lib` addition (`ESNext.Iterator`). Impact on this ticket: **none on design** — `prompt-questions.ts`, `lib/active-ticket.ts`, `lib/quality-state.ts`, `prompt-timestamp.ts` were all untouched by main. Two build-phase notes:

- The new length-cap **test** lives in `tests/` and lints under ESLint 10 + unicorn 68 — follow merged conventions (no abbreviations: `length` not `len`, `message` not `msg`; consolidate mutable test state into a `const` object, per the merged `hierarchy.test.ts` pattern). Templates themselves aren't linted by `eslint src tests`, but keep new template code abbreviation-free for consistency.
- **Parity:** edits to `packages/cli/templates/hooks/prompt-questions.ts` must be mirrored to the `.safeword/hooks/` dogfood copy (parity test enforces). Run `cd packages/cli && bun run lint` (not just root lint) before committing.
...
- The `prompt-questions.ts` reminder surfaces a compressed pointer to all five dimensions (intent / done / constraints / riskiest-assumption / request-shape) during Clarify — verifiable in the hook's emitted output — and fires in **both** the active-intake-ticket branch **and** the `No active ticket` (pre-classify) branch.
- The pointer line stays within the length cap (≤~20 words) — a test asserts it doesn't bloat into spelled-out prompts.
...
All resolved (via /figure-it-out, grounded in safeword's instruction-attention learnings):

- ~~**Soft or firm surfacing?**~~ _Resolved: **hook pointer + SAFEWORD.md prose**, not either/or. `instruction-attention-hierarchy.md` — the hook reminder is ~95% and the only channel that fires when work is scoped without a skill; SAFEWORD.md-only is the current dormancy (~50% mid-file / ~20% cross-file)._
- ~~**Does 3→5 per-turn cross into interrogation?**~~ _Resolved: no, because the reminder is **Clarify-phase-gated** (not a 50-turn banner) and a **compressed keyword pointer** (not five spelled-out prompts). `long-session-style-drift.md:54`'s per-turn-reinjection anti-pattern is about standing style rules; this is a phase-scoped trigger that must hit the prompt to beat dormancy._
- ~~**Ledger shape?**~~ _Resolved earlier: settled by V6N5PW (reuse the `## Open Questions` spec section; heavier brief deferred)._

Residual risk (tracked, not a blocker): the compressed pointer must fit five dimensions in ≤~20 words without bloating toward a checklist or collapsing into a vacuous "check readiness". Mitigation = terse keyword list pointing to SAFEWORD.md for phrasing; enforce the line-length cap as a test.
...
- 2026-06-21T04:37Z /figure-it-out resolved the surfacing + interrogation open questions using safeword's own instruction-attention learnings. Resolution: hook compressed-pointer + SAFEWORD.md prose (NOT either/or) — hook is ~95% and the only channel that fires when work is scoped without a skill; SAFEWORD.md-only IS the current dormancy. Interrogation fear dissolved: reminder is Clarify-phase-gated (not a 50-turn banner) + compressed keyword pointer (not 5 prompts). Surfaced an implementation wrinkle now in scope: the pointer must also fire in the `No active ticket` pre-classify branch (the motivating first-turn case), not only once an intake ticket exists. Added length-cap test to done_when. All open questions now resolved → ready for implement pending user go-ahead.
- 2026-06-21T04:42Z Caught up to main (merge 5fbe506) + ran `bun ci` (exit 0). /figure-it-out impact assessment: NO design/scope change — main left all ticket core files untouched (prompt-questions.ts, active-ticket, quality-state, prompt-timestamp). ESLint 9-vs-10 is per-package (cli=10+unicorn68, root=9), not a bug; tsconfig change cosmetic (ESNext.Iterator). Added two build-phase implementation notes (new test follows unicorn-68 conventions; template↔dogfood parity + lint cli package). Ticket remains implement-ready.
