...
source: Anthropic prebuilt skill — pasted by user 2026-06-21, not present on this container's /mnt/skills
---
...
1. **Frame** — What are we exploring? Why now? What do we already know? What are the constraints? What would a great outcome look like? (Spend enough time here.)
2. **Diverge** — Generate many ideas, no judgment. Push past the obvious first 3-5. Build on ideas; follow tangents.
3. **Provoke** — Challenge and extend. "Strongest argument against this?" "Who would hate this?" "What are we not seeing?" "10x more ambitious version?"
4. **Converge** — Group into themes; evaluate on user impact, feasibility, strategic alignment, evidence strength. Don't kill by committee. Identify top 2-3; for each, name the biggest unknown and the cheapest way to resolve it.
5. **Capture** — Key ideas + why interesting; assumptions to test; questions to research; next steps; what was explicitly set aside.

## Being a Good Thinking Partner

Do: be opinionated; challenge constructively ("That assumes X — are we confident?"); bring unexpected angles; match energy; ask the next question; name the pattern (solutioning too early, scope creep, feature parity).

Do Not: dump frameworks; generate a list and hand it over; agree with everything; optimize prematurely (don't evaluate feasibility in divergent mode); anchor on the first idea; confuse brainstorming with decision-making.

## Common Anti-Patterns

- **Solutioning before framing** — jumps to "build X" before defining the problem. Ask what user problem X solves and how we know.
- **The feature parity trap** — "competitor has X so we need X." Ask what user need X serves.
- **Anchoring on constraints** — explore freely first, then feasibility.
- **The one-idea brainstorm** — a solution dressed as a brainstorm. Acknowledge, then push for alternatives.
- **Analysis paralysis** — too much exploration, no convergence. "If you had to pick one right now, which and why?"
- **Brainstorming when you should be researching** — some questions need data, not ideation. Stop and identify what research is needed.
