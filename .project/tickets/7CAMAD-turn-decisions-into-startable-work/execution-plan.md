...
3. RED: In the same journey, change only independent task order and assert only
   ...
   byte-identical A639WN class with no new proof-execution receipt. The installed
   approval-command boundary remains in `plan-design-approval.test.ts` and is
   included in the `replan-journey` proof below.
...
| contract-identity | command | integration | A disposable installed CLI names missing, stale, and incomplete author/reviewer contract copies while admitting the exact packaged pair | real_boundary | current_required | {"type":"command","cwd":"packages/cli","argv":["bun","run","test","tests/review/execution-plan-contract-identity.test.ts"]} |
| replan-journey | command | E2E | Git-backed installed CLI completes cold start, RED handoff, both replan routes, evidence preservation/demotion, resume, and rollback while the supporting corpus asserts every semantic partition | real_boundary | current_required | {"type":"command","cwd":"packages/cli","argv":["bun","run","test","tests/integration/plan-execution-journey.test.ts","tests/integration/plan-design-approval.test.ts","tests/integration/delivery-checklist-cli.test.ts","tests/review/execution-plan-conformance.test.ts"]} |
