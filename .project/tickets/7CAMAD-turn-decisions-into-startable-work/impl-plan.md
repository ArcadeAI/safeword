...
| Implemented | The canonical BDD workflow, Implementation and Execution Planning guides, handbook phase model, planning-gates architecture record, templates, and registered Claude, Cursor, Codex, and OpenCode derivatives carry the accepted contract. | Parity and generated-artifact gates fail when a canonical source and its registered derivatives diverge. | Whole-ticket verification only. |
...
both planning reviews. The reviewed-definition and ordinary-progress fields
follow the asymmetric Execution Plan identity recorded under Decisions. A stale
...
| Obligation, checklist, evidence, and PR-slice completeness    | Existing plan-execution record and Delivery Checklist admission                                                                                           | Semantic review covers omitted and partially mapped applicable obligations, explicitly inapplicable optional categories without placeholder work, known-defect versus target state, pending human authority, and canonical A639WN/6XW8H7 contract identity | Existing parser, normalized-digest, proof-currency, checklist, and slicing matrices regenerated whenever the canonical contract digest changes | Structural admission cannot establish semantic relevance by itself                                          |
| Measurement work preserves the accepted promise               | Execution Planning semantic contract                                                                                                                      | Installed-CLI semantic review covers every accepted R15 measurement partition without changing the upstream target, validity safeguards, or failure behavior | Packet completeness and contract-identity tests                                                                       | No quantitative product promise exists for this child itself                                                |
| Dependency-directed invalidation and replanning               | Coding authorization, review identities, and A639WN's `delivery-compatibility` review plus durable `delivery-compatibility:v1` decision event             | Git-backed integration separately varies admitted Product Plan identity, Implementation Plan bytes, Execution Plan `Required proof`, contributor progress cells, and a disposition-class transition; only progress retains review currency, and an Execution-only change leaves the upstream Implementation Plan review current | Mutation tests assert preserved proof keeps the byte-identical evidence class without a new proof-execution receipt, while invalidated proof becomes audit-only | Exact-byte upstream identity can require review for cosmetic edits; that conservative cost is deliberate    |
...
2. First prove installed-CLI admission and mismatch rejection for the canonical
   ...
   semantic-conformance matrix across the accepted R4 discovery-routing, R6
   implementability, R10 obligation-mapping, R13 PR-slicing, and R15
   measurement partitions defined in the Execution Plan. Bind
   the admitted matrix to the same canonical contract digest, regenerate it
   whenever that digest changes, and reject a matrix generated from superseded
   contract bytes; keep the individual case inventory out of this decision
   record.
...
| Safeword Maintainer   | Maintains one prerequisite evaluator and one authorization projection rather than duplicated hook logic; a source assertion fails if either consumer computes a prerequisite itself. Approval and user-facing recovery are not applicable maintainer responsibilities here because this command grants no human authority and K3EBHB owns recovery rendering. | The hook still depends on resolving and invoking a healthy local CLI |
...
further data reversal. The reversal is resumable and idempotent: an interrupted
run leaves unmoved tickets conservatively gated, and a retry rereads current
ticket state before applying the same phase change.
...
| Reuse content-bound review identity for dependency-directed replanning    | Exact upstream plan changes invalidate dependent authorization. The complete disposition set is `open`, `complete`, `not_applicable`, and `pending_human`. Each reviewed and current snapshot is normalized independently from its own disposition: identity excludes `Disposition`, `Evidence class`, `Revision`, and final evidence only for rows that are `open` or `complete` in that snapshot. Therefore every transition between an ordinary-progress class and `not_applicable` or `pending_human` is identity-bearing and stales the review. Every byte of `not_applicable` and `pending_human` rows remains identity-bearing. All other plan content, including checklist `ID`, `Category`, `Obligation`, `Owner`, `Required proof`, applicability reasons, human dependencies, and task order, remains identity-bearing. Unchanged proof retains its class and proof invalidated by a changed decision becomes audit-only. | Add an invalidation ledger or mutable stale flags; restart all work; preserve every proof | Existing review identity already represents currentness; another store creates synchronization failure, while full restart loses valid work and indiscriminate preservation overstates completion |
...
| Require explicit skips for each planning phase                            | `plan-implementation` and `plan-execution` are separate contracts, so omitting either requires its own reason in `phase_skips`                                                             | Let an Implementation Planning skip implicitly skip Execution Planning during migration | The shim could silently bypass startability review even when only design review was intentionally omitted; existing tickets must make the second exception explicit |
| Preserve one real behavior obligation without manufacturing optional work | Every approved feature retains at least one accepted behavior owner and reviewable slice; migration, rollout, rollback, documentation, and affected-surface work may be absent only when the accepted approach explicitly marks each category inapplicable | Add a zero-slice `no_pull_request` mode; require placeholder tasks for every category | Zero slices contradict the accepted feature and slicing contracts and could authorize planning with no executable work; placeholder tasks misrepresent deliberately inapplicable work |
...
Exact-byte Implementation Plan identity and normalized Execution Plan identity
are the integrity boundary for deciding which review remains current.
...
| Discover decisions before prescribing work        | Execution work may consume accepted decisions but must return any changed design or proof boundary to Implementation Planning | [R4/R6 contract](features/turn-decisions-into-startable-work.feature); conformance mutations distinguish execution-only, architecture, data, proof, and API-contract changes |                   |
...
- **Every feature requires current planning before coding:** Legacy feature
  tickets with no current or historical M1 marker remain outside this new gate
  until YCFFNC migrates them. This avoids retroactively blocking in-flight work;
  new features are still held by the existing planning code freeze before any
  marker exists, then by coding authorization once they enter Execution
  Planning. Any marker with unresolvable provenance fails closed rather than
  regaining the legacy exemption.
