...
1. Vendor the minimal MIT-attributed loopback protocol fixture for a disposable
   local OpenCode 1.18.23 proof. Observe `bash`, `shell`, `edit`, `write`, and
   `patch` reaching `tool.execute.before` with the approved input keys, verify a
   hand-written plural command/agent plus `.claude/skills` entry are discovered,
   and observe `client.global.health()` returning exactly `1.18.23`. In one
   decisive run the loaded plugin spawns and awaits a Node/Bun-compatible fixture
...
4. Generate the stable profile plugin and canonical dispatcher transport. It
   ...
   single unavailable-binding state. Plugin initialization obtains the optional
   OpenCode version once from `client.global.health()` and omits it when that
   local SDK call fails. Marker lookup is re-run per event with a 50 ms fail-open
   deadline. Only the first handled hook after confirmed enrollment performs a
   100 ms fail-open health race and caches the settled optional version; injected
   hangs prove unmarked projects make no SDK call. The allow arm reuses the
