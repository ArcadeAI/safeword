...
| `/quality-review` requirement | Computed loop-count-aware at the call site                               | Add `quality-review` as a static `PHASE_GATES.done` entry | Static entry blocks _every_ done, including single-loop tickets — violates TB2 (silent gate)          |
