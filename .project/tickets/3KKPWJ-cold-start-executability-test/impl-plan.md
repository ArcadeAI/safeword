...
| Instruction home  | A **guide** (`.safeword/guides/cold-start-check.md`) | A registered skill (`skills/cold-start-check/`) | Survey found a skill needs 5-surface parity wiring (template + .claude + .agents + schema×2 + skill-cursor-pairs fixture + cursor command) — too heavy for an advisory, rarely-fired check; a guide is 1 surface + 1 schema entry with identical content. Premortem demanded near-zero build cost. |
