...
- 2026-06-25 Offender identified (data point for the per-file breakdown): `tests/integration/cucumber-bdd.test.ts > dogfood_feature_runs_green` is the lane that breaches under saturation. It spawns `bunx cucumber-js` (tsx-transpiles step defs + runs all features) inside the forks pool against a 60s `TIMEOUT_SETUP`. Measured this session: **~52s isolated** (passes), **77s under back-to-back full-suite load** (times out) — the exact CPU-saturation race this ticket already documents. Thin 8s solo headroom even unloaded. `/figure-it-out` call (option A): leave it for this ticket's two-tier fix rather than a blunt timeout bump (which would mask creep) — the dedicated CI runner doesn't hit it, and CI also proves the lane separately via `bun run test:bdd`. When the two-tier pools land, route this spawn into the sequential tier. (Not part of the #82cc0r P1 batch; logged from that session.)
