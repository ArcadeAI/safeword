...
| Host classification | JSONC-safe parse nearest supported Biome config; Ultracite preset wins | Detect package dependency only; direct Biome always wins | Dependencies are ambiguous, JSON.parse rejects supported configs, and precedence would violate existing Ultracite setups. Evidence: [current Ultracite CLI and preset documentation](https://github.com/haydenbleasel/ultracite). |
