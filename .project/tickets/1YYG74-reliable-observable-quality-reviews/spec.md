...
- `data.preferred_failure` admits `not_installed`, `unsupported`, `probe_timed_out`, `not_authenticated`, `launch_failed`, `process_failed`, `timed_out`, `invalid_output`, `REVIEWER_PROVENANCE_MISSING`, `REVIEWER_PROVENANCE_CONTRADICTORY`, and `source_changed`; route exhaustion is the separate machine-stable finding code `REVIEW_ROUTES_EXHAUSTED`, and both layers use action-required exit 2.
