...
- **Command observer:** Every scenario phrase stating that no destructive command is run or emitted is an assertion against an installed observer covering branch, worktree, merge, approval, and pull-request state-changing commands; it is never inferred from the absence of a crash or continuation.
...
- **Validation precedence:** For writes, prove the acting task is named by the current profile activation marker before inspecting handoff or claim ownership; a task without that authorization always receives the same stale-protection result regardless of its ownership history. A current task rewriting the same identity it already claims is idempotent and preserves both records. For discovery, scope candidates by normalized repository store key before reporting record state, without reading unrelated entries; within matching candidates, reject unsafe paths and unreadable structure before decoding identity, then validate decoded repository agreement, profile provenance, and time. More than one match is ambiguous if any is usable. If every matching record is unusable, discovery reports inert unusable state without mutation and an authorized write may replace all of them with one fresh handoff.
- **Cleanup observation precedence:** Reject unsafe or foreign repository identity first, then unavailable or mismatched pull-request identity/head, then an unmerged pull request, then recreated target identity, then missing individual targets. A merged pull request with both preview-recorded branch and worktree targets absent is already cleaned: emit no destructive command and remove the surviving advisory receipt and claim.
