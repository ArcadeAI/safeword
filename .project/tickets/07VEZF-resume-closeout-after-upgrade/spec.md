...
- **Repository matching:** A decodable record matches only when both its normalized store key and its decoded canonical repository identity match the current repository. An undecodable record matches by normalized store key alone, so malformed state cannot evade ambiguity or explicit replacement handling.
