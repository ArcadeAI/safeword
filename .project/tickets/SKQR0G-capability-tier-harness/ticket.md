...
- 2026-06-23T17:25Z /figure-it-out: **does Cursor/Codex parity impact these children?** (live docs verified). Verdict: **parity is content-parity + graceful degradation, not capability-parity — it doesn't change *which* children are relevant, it constrains *mechanism* for two.** Corrected a stale assumption in the process: all three harnesses now have subagents + parallel fan-out + per-subagent model (earlier safeword guidance saying "Codex subagents never auto-spawn" predates Codex's subagents feature). **9BDDGP** verifier fan-out → must be skill-guidance with per-agent degradation; sole divergences are Codex **user-invoked** spawn (+ `max_depth=1`) and Cursor **observe-only** `preCompact`. **P30CRP** reinjection → strong on CC/Codex (`PreCompact`+`PostCompact`), weak on Cursor (observe-only). **Z4Q24Q** cross-model review → *better* supported than assumed (Codex exposes per-subagent `model`). **QSNKBB/ZBVGPF** → unchanged. Full corrected capability matrix logged to 9BDDGP; capability-gap notes added to the changelog-alignment epics (QM5G9M Codex subagents, VAX3Z2 Cursor compaction, 8R54HV CC compaction). The changelog epics were found *ahead of* safeword's deployed wiring on hooks — so the parity work is "wire what the epics already mapped," not new research.
