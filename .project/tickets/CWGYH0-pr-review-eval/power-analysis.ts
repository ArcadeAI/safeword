...
function caseDifference(random: () => number, scenario: Scenario): number {
  ...
  for (let trial = 0; trial < TRIALS_PER_CASE; trial += 1) {
    const draw = random();
    if (draw < scenario.aOnly) {
      difference += 1;
    } else if (draw < scenario.aOnly + scenario.bOnly) {
      difference -= 1;
    }
  }
  return difference / TRIALS_PER_CASE;
}
...
function estimatePower(caseCount: number, scenario: Scenario): number {
  const random = mulberry32(
    SEED + caseCount * 1_009 + Math.round(scenario.aOnly * 10_000)
  );
  let passes = 0;
  for (let experiment = 0; experiment < EXPERIMENTS; experiment += 1) {
    const differences = Array.from({ length: caseCount }, () =>
      caseDifference(random, scenario)
    );
...
const zeroFixedHitProbability = CASE_COUNTS.flatMap((caseCount) =>
  [0.005, 0.01, 0.02].map((perTrialRate) => ({
    caseCount,
    perTrialRate,
    probabilityOfZeroNamedHits: (1 - perTrialRate) **
      (caseCount * TRIALS_PER_CASE),
  }))
);
