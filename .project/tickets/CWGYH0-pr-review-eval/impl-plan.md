...
1. Extract one exhaustive `TrialDisposition` result in `scored-run-policy.ts`: `{status: "usable", reason: "completed", retry: "never"}` or `{status: "invalid", reason: <canonical reason>, retry: "infrastructure-once" | "never"}`. Runner, writer, scorer, and tests consume that result without reclassification. Unknown or absent evidence is invalid. Add RED/GREEN tests for findings, explicit empty findings, every provider/reviewer/provenance failure, and retry class.
2. Establish the minimal final writer lifecycle before the wiring proof. Replace direct writes to `active/` with provisional case directories and an explicit versioned state record. The runner is single-process and sequential; an exclusive run lock rejects concurrent writers. Each attempt is written to a temporary file, flushed, and atomically renamed before state advances. State updates use temporary-write, file flush, rename, and containing-directory flush. A completed case directory is renamed once into `active/`; an unusable case is renamed once into `quarantine/`, then the incremented reserve index and allocated identity are durably recorded before any replacement provider call. Resume reconstructs only from the state record plus sealed directories and makes the transition idempotent at every injected crash boundary.
...
7. Add a canary-gate module that consumes individual fixture, operational, paid-outcome, provenance, mechanical-label, and cost records. Its authorization payload binds SHA-256 identities for runner, `TrialDisposition` classifier, writer, scorer, fixtures, labels, manifest anchor, provider/model configuration, cost policy, preregistered matrix, derived effective matrix, source commit, run ID, and exactly one next checkpoint. Before every gated provider call, the runner recomputes and matches the payload; any change or replay against another run/checkpoint fails closed.
8. Freeze the disjoint ten-call canary inputs, mechanical labels, genuine-empty designation, R1 fixture taxonomy, R2 operational taxonomy, cost ceilings, and immutable manifest anchor. Run the no-cost suite only. Paid execution remains a separate explicit step after review of green evidence.
