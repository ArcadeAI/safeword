...
7. Add a canary-gate module that consumes individual fixture, operational, paid-outcome, provenance, mechanical-label, and cost records. Its authorization payload has one exact binding inventory: runner, `TrialDisposition` classifier, writer, scorer, effective-matrix derivation code, fixtures, labels, provider/model configuration, cost policy, preflight bytes, primary and reserve manifests, preregistered matrix, source-commit inventory, run ID, and exactly one next checkpoint. The gate envelope itself is loaded from an immutable Git object named by an unedited external issue anchor, avoiding an impossible self-digest inside the payload. Before every gated provider call, the runner recomputes and matches the payload; any missing or extra binding, change, or replay against another run/checkpoint fails closed. Derived output and its raw-artifact manifest do not exist before paid calls, so after the run the scorer separately requires their immutable external anchor and verifies every input byte before deriving the effective matrix.
