...
- [x] Threat-model note written in `lib/update-cache.ts` (layered defenses + accepted residual + revisit trigger); provenance verification explicitly deferred, no sigstore code.
...
- 2026-06-20T15:44:00Z Final gate before PR. **/verify** green: test suite 3108 pass / 5 skipped (208 files), build ✅, lint ✅ (eslint + gherkin-lint + tsc), dep-drift ✅ (no new deps); Gherkin acceptance lane N/A (task, no new scenarios). **/audit** passed (XQ9CXA scope, 0 errors / 0 warnings): architecture clean (depcruise, no violations), no dead code (knip), 0 duplication clones (jscpd, gitOk extraction helped), test quality clean (22 strike-counter tests — specific assertions, no weak/arbitrary-wait, edge cases covered). Also ran **whole-ticket quality-review** (holistic fresh-model pass on the cumulative diff → fixed orphaned update-check deprecatedFiles + success-path isolation) and **/refactor** (FETCH_TIMEOUT_MS naming, gitOk extraction). Branch caught up to origin/main (rebased, re-signed). Ready for PR.

- 2026-06-20T08:03:00Z Item #3 DONE (doc-only). Revalidated via /figure-it-out → defer client-side provenance verification, document the threat model. Confirmed stronger than before: the #2 review added two layers (semver-validated `latest` + `execFileSync` no-shell). Verified no free client verification (bun does not verify npm provenance on install per web search; the path is out-of-band npm/sigstore/cosign). Wrote a layered "Supply-chain threat model" note in lib/update-cache.ts (postinstall RCE→bun-no-scripts; tampered tarball→integrity hash; injection→semver+execFileSync; yank→24h cooldown; accepted residual = valid-but-malicious publish, provenance published-but-not-client-verified) with the revisit trigger. Green: typecheck, unit 22, release/parity 7. **All three ticket items complete.**
- 2026-06-20T07:58:00Z Quality-review pass on item #2 (pushed 0dea63d). Independent sonnet reviewer confirmed strike-counter correctness (no off-by-one, message-once, survives refresh, no exit race) + git pre-flight. Fixed the one finding: upgrade now runs via `execFileSync('bunx', [...])` (no shell) + semver-validates registry `latest` — closes a command-injection surface. Rejected `!= null` nit (cosmetic).
