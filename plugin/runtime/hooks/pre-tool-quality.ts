...
// enforces everywhere; a phase list enforces only where it is named (2VCSZY).
...
// via `write-review-stamp.ts --phase`. Inert unless `reviewGate` names this exit:
// `true` covers every phase, a list covers only the phases it names (2VCSZY).
if (isCanonicalTicketEdit) {
  const context = canonicalTicketEditContext();
  ...
  if (exitedPhase !== undefined && reviewGateAppliesTo(exitedPhase)) {
    ...
    if (!gatePhaseAdvance(phaseScope, stamps, crossAgentReviewPolicy()).ok) {
      deny(
        ...
        `Run the phase's \`safeword review run\` command, then record its author_agent, actual_reviewer, and independence with \`bun "\${CLAUDE_PLUGIN_ROOT}"/runtime/hooks/write-review-stamp.ts --phase ${exitedPhase}\`; add a model only when independently verified. To stop gating this exit, narrow \`reviewGate\` in .safeword/config.json to the phases you want (or set it to false).`,
      );
    }
