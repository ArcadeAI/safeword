...
2. **A fresh-context review.** Resolve a review-capable Safeword CLI, then run the shared coordinator with only the bounded design evidence:
   ...
   ```bash
   bun "${CLAUDE_PLUGIN_ROOT}"/runtime/hooks/run-review.ts review run plan-implementation --agent-handoff --json --context ticket-path/spec.md ticket-path/ticket.md feature-file principles-file personas-file surfaces-file architecture-records -- ticket-path/impl-plan.md
   ```
