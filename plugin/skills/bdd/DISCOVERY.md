...
Use `/demand-research` only when Why now contains an unresolved,
...
`/safeword:verify` walks the Payoff as the persona before the ticket closes — so a Payoff
...
tagged `@surface.<slug>` or an inline `skip: <reason>`, and `/safeword:verify` records
