...
run the coordinator and every returned `nextActions` command inside the current
sandbox. Never set `sandbox_permissions: "require_escalated"` or request host
approval for `review run`, `review status`, retries, or fallbacks. The active
Codex permission profile owns any reviewer network access. If that normal
sandbox cannot reach the configured reviewer, report the route as unavailable;
do not escalate it.
