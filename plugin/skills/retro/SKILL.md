---
name: retro
description: Run a safeword retrospective on the current session on demand — mine the
  session transcript for qualitative safeword friction (bugs / rough edges / gaps) and
  file it upstream behind the egress guard. Use when the user says "run a retro",
...
- **In-context (any harness):** extract the friction yourself following
  `"${CLAUDE_PLUGIN_ROOT}"/resources/guides/retro.md` (fresh-context read of the transcript → structured findings
...
- **Lane 2 (no token / cloud):** the CLI spools the sanitized, post-egress drafts and a
  nudge surfaces them; file each **verbatim** per `"${CLAUDE_PLUGIN_ROOT}"/resources/guides/self-report-filing.md`
