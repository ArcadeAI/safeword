...
**End with a self-contained call.** For structured replies (verdicts, reviews, recommendations, audits), close with a literal `**Next:**` paragraph; the stop hook captures it verbatim for the session's re-entry brief. Write for a reader who sees only that paragraph. Under `terminal-handoff/v1`, a human decision uses exactly `Choice: <concrete choice>. Recommendation: <recommended option>. Reason: <controlling reason>. Impact: <material tradeoff or consequences>. Reply: <exact reply>.` Explain any necessary marked term as `Term: <name> = <plain-language meaning>.` If no decision is required, use exactly `Action: <imperative + specific object>.` and optionally only `Reason: Required because <essential reason>.` Use specific nouns, verbs, paths, commands, amounts, and consequences. Include a detail only if it could change the decision or action. Stop once the reader can act without scrolling. Short conversational replies can end with the question itself.
