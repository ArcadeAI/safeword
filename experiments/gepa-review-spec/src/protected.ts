...
import { defectFamily, type DefectType, type ExpectedDefect } from './types';
...
/**
 * The floor-breaching misses: seeds the candidate missed AND the baseline
 * protects. A `null` protectedSet (no manifest) → strict: every miss counts,
 * the original absolute floor (backward-compatible). An unknown fixture (absent
 * from the manifest) is also strict — over-guard rather than under-guard.
 */
export function protectedMisses(
  s: FixtureScore,
  protectedSet: ProtectedSet | null,
): ExpectedDefect[] {
  if (protectedSet === null) return s.falseNegatives;
  const prot = protectedSet.get(s.name);
  if (prot === undefined) return s.falseNegatives;
  return s.falseNegatives.filter(e => prot.has(seedKey(e)));
}
