...
- **Coverage adequacy?** Consult testing guide's bug detection matrix. Ask: "What could still break that this test wouldn't catch?" Flag gaps — missing edge cases, error paths, or boundary values. **Where a gap goes:** a missing scenario is a scope change, not a mid-implement edit — defer it to a follow-up ticket, or loop back to define-behavior, re-run the scenario-gate, revise and re-review the Implementation Plan, then refresh and re-review the dependent Execution Plan before resuming implementation. Don't silently append scenarios to a signed-off `test-definitions.md`.
