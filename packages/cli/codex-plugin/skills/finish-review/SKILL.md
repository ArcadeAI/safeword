---
name: finish-review
description: Use only right after the shared review coordinator reports it's run
  out of reviewer routes; a bounded internal fallback, not something a user
  invokes directly.
---
...
- Codex: invoke one fresh-context in-session subagent when the host exposes that
  ...
  `references/REVIEWER.md` contract with
...
matches `references/REVIEWER.md`. Unavailable capability,
...
evidence, return to `$safeword:pr-readiness` after reporting it. Apply or answer every
