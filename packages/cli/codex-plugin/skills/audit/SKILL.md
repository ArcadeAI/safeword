...
NS_ROOT="$(bunx --bun safeword@0.79.2 project namespace-root --cwd "$PROJECT_DIR" 2> /dev/null)"
[ -d "$NS_ROOT" ] || {
  if [ -d "$PROJECT_DIR/.project" ]; then NS_ROOT="$PROJECT_DIR/.project"; else NS_ROOT="$PROJECT_DIR/.safeword-project"; fi
}
PERSONAS_FILE="$(bunx --bun safeword@0.79.2 project namespace-root --cwd "$PROJECT_DIR" --key personas 2> /dev/null)"
SURFACES_FILE="$(bunx --bun safeword@0.79.2 project namespace-root --cwd "$PROJECT_DIR" --key surfaces 2> /dev/null)"
GLOSSARY_FILE="$(bunx --bun safeword@0.79.2 project namespace-root --cwd "$PROJECT_DIR" --key glossary 2> /dev/null)"
