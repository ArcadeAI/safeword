...
`bunx --bun safeword@0.81.1 project review-knowledge --json` and read the current
...
```bash
SAFEWORD_REVIEW_PROGRESS=1 bunx --bun safeword@0.81.1 review run scenario-gate feature-file [legacy-test-definitions] --context ticket-spec [dimensions-file] principles-file personas-file surfaces-file --agent-handoff --json
```
