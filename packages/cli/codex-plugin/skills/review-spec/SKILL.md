...
```bash
SAFEWORD_REVIEW_PROGRESS=1 bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/0.83.1/runtime/cli.js" review run scenario-gate feature-file [legacy-test-definitions] --context ticket-spec [parent-spec] [dimensions-file] principles-file personas-file surfaces-file --agent-handoff --json
```
