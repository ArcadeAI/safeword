...
description: Explore and debate options with fresh documentation and research
  ...
  intent (elicit), or reviewing already-written code (quality-review).
---
...
A real decision with two or more plausible directions: library or framework choice, architecture, API or schema design, algorithm selection, communication strategy, policy call. User invokes `$safeword:figure-it-out`.
...
One sentence: what are we deciding, and what would make a choice wrong? If you can't write it, call `$safeword:elicit` — the option space is undefined.
