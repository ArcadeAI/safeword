...
!`PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}" && CLAUDE_PROJECT_DIR="$PROJECT_DIR" bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/1.0.0-rc.1/runtime/cli.js" project runtime write-review-stamp -- spec`

If no `[skill-invocation-log] ... ✓` line appears above, run this fallback before stopping:

```bash
PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2> /dev/null || pwd)}"
CLAUDE_PROJECT_DIR="$PROJECT_DIR" bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/1.0.0-rc.1/runtime/cli.js" project runtime write-review-stamp -- spec
```
...
review time, run `bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/1.0.0-rc.1/runtime/cli.js" project review-knowledge --json` and use its
...
```bash
bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/1.0.0-rc.1/runtime/cli.js" project runtime write-review-stamp -- spec --skip "<why this spec needs no review>"
```
