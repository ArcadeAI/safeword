...
2. **Derive dimensions** from intake artifacts (resolved questions, `done_when`, `scope`) + domain-knowledge dimensions not surfaced during intake. Read `out_of_scope`, the Product Bet's project non-goals, and the applicable milestone's Non-goals in the same pass and hold all three as the outer edge — a child feature's `spec.md` carries no spec-level non-goals by design, so `out_of_scope` is its local edge while inherited project and milestone non-goals remain part of the edge. A dimension or partition that exists only past any of those lines is dropped here, before partitioning — not carried forward and not "covered for completeness". Domain knowledge is the usual source of the overshoot, because it arrives unbounded by intake.
3. **Partition and place proof** — identify equivalence classes and boundaries, then mark each as a distinct externally meaningful behavior or recovery path, a lower-level variation of an already covered behavior, or excluded by scope. Record the placement in `dimensions.md`. A long input matrix is not a long scenario list.
4. **Generate scenarios** — choose the smallest representative set that distinguishes each Rule's user-visible outcomes, important boundaries, and independently meaningful failures. Do not generate one scenario per input partition or boundary by default. Put repeated schema fields, format variants, arithmetic cases, and malformed-input permutations in table-driven lower-level tests; name their planned proof in `dimensions.md` for the Execution Plan to carry forward. Each scenario proves a specific **Rule** (or legacy Acceptance Criterion) from intake (`spec.md`); if it does not, drop an implementation-only case or return to intake for a missing criterion. A real Rule does not override `out_of_scope` or project and milestone Non-goals; name a crossing and let the user decide whether to amend scope or drop the scenario.
5. **Organize and compress under Gherkin `Rule:` blocks**. Every Rule needs a falsifiable example, and no scenario is an orphan. For each scenario, ask whether removing it would lose a distinct user-visible outcome, recovery path, or boundary that another scenario cannot prove. If not, remove it and keep its useful input variation in the lower-level test plan. Do not merge different obligations merely because the current implementation breaks them together. Trim duplicate lower-level cases before presenting the set, and update the R/G/R ledger when editing a saved feature.
