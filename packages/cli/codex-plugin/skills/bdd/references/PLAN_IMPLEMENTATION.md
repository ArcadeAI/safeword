...
1. **Independent review first.** At review time, run `bunx --bun safeword@0.79.5 project review-knowledge --json`. Resolve a review-capable Safeword CLI, then invoke the coordinator with the current files identified by the resolver:

   ```bash
   SAFEWORD_REVIEW_PROGRESS=1 bunx --bun safeword@0.79.5 review run plan-implementation impl-plan.md spec.md ticket.md feature-file principles-file personas-file surfaces-file --agent-handoff --json
   ```
