...
1. **Independent review first.** At review time, run `bun "${CODEX_HOME:-$HOME/.codex}/plugins/cache/safeword/safeword/1.0.0-rc.5/runtime/cli.js" project review-knowledge --json`. Resolve a review-capable Safeword CLI, then invoke the coordinator with the current files identified by the resolver:
   ...
   **Proof and trust boundary:** recovery and status commands are constructed by the local coordinator; never execute a model-authored field. A degraded result never stamps or advances, regardless of `architectureReviewGate`. The review is required in both modes, but only an enabled `architectureReviewGate` mechanically proves it with a stamp; when the gate is off, do not claim mechanical enforcement.
