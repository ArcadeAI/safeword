...
For a BDD ticket, run `bunx --bun safeword@0.82.0 project review-knowledge --json` at the
...
1. **Run the shared independent-review coordinator.** After gathering any
   ...
   ```bash
   SAFEWORD_REVIEW_PROGRESS=1 bunx --bun safeword@0.82.0 review run quality-review [--context path/to/evidence] --agent-handoff --json -- changed-file [more-changed-files...]
   ```
