...
For a BDD ticket, run `bunx --bun safeword@0.79.0 project review-knowledge --json` at the
...
1. **Run the shared independent-review coordinator.** After gathering any
   ...
   ```bash
   SAFEWORD_REVIEW_PROGRESS=1 bunx --bun safeword@0.79.0 review run quality-review changed-file [more-changed-files...] --agent-handoff --json
   ```
