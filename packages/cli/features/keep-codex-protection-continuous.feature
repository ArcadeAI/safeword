...
Feature: Keep Codex protection continuous during profile-plugin migration
  ...
  Rule: codex-continuity.TBU1.R2 — Plugin readiness requires current hook-execution proof, not installation or enablement alone
    ...
    Scenario: Successful installation requires a Codex restart
      ...
      Then migration reports plugin_installed_restart_required and changes no repository file
      And the profile contains a restart marker bound to the installed version and hook manifest

    Scenario: Plugin SessionStart clears restart-required state
      Given a profile with a current restart-pending marker
  ...
  Rule: codex-continuity.TBU1.R4 — Shared cleanup is explicit, selective, recoverable, and idempotent
    ...
    Scenario Outline: Repeated migration converges in every pre-finalization state
      ...
      Examples:
        ...
        | plugin_disabled               | plugin_installed_restart_required |
        | plugin_setup_required         | plugin_installed_restart_required |
        | plugin_installed_restart_required | plugin_installed_restart_required |
        | plugin_enabled_hook_unproven  | plugin_enabled_hook_unproven    |
        | compatibility                 | compatibility                  |
        | not_configured                | plugin_installed_restart_required |
  ...
  Rule: codex-continuity.NTB1.R1 — Every migration state names protection and one next action
    ...
    Scenario Outline: Human status gives one safe next action for settled migration states
      ...
      Examples:
        ...
        | restart pending without legacy | plugin_installed_restart_required | unprotected | restart Codex and review /hooks |
        | restart pending with complete legacy | plugin_installed_restart_required | protected | restart Codex and review /hooks |
        | restart pending with partial legacy | plugin_installed_restart_required | partial | restart Codex and review /hooks |
    ...
    Scenario Outline: JSON status uses state-specific complete schema
      ...
      Examples:
        ...
        | restart pending without legacy | plugin_installed_restart_required  | unprotected | 1            | restart Codex and review /hooks    | 2         |
        | restart pending with complete legacy | plugin_installed_restart_required | protected | 1            | restart Codex and review /hooks    | 2         |
        | restart pending with partial legacy | plugin_installed_restart_required | partial | 1            | restart Codex and review /hooks      | 2         |
        | complete legacy                | legacy                             | protected   | 1            | safeword codex migrate             | 2         |
        | partial legacy                 | legacy                             | partial     | 1            | safeword codex migrate             | 2         |
        | unproven without legacy        | plugin_enabled_hook_unproven       | unprotected | 1            | restart Codex and review /hooks    | 2         |
        | unproven with legacy           | plugin_enabled_hook_unproven       | protected   | 1            | restart Codex and review /hooks    | 2         |
  ...
  Rule: codex-continuity.SWM1.R2 — A finalized repository retains a small plugin-setup bootstrap without duplicated workflow policy
    ...
    Scenario: New teammate receives only the plugin setup path
      ...
      Then it explains install, restart, hook review, and status without embedding workflow policy
