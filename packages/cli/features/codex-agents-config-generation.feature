...
Feature: Codex agents config generation
  ...
  Safeword setup and upgrade generate Codex-readable project config without
  reinstalling reusable Codex implementation files.
  ...
  Rule: Setup installs Codex project assets
    ...
    Scenario: codex-agents-config-generation.SM1.AC1.fresh_setup_creates_codex_assets
      ...
      Then the project has `.codex/config.toml` with packaged Safe Word hook commands and no repo-local Codex implementation files
  ...
  Rule: Upgrade preserves user-owned Codex config
    ...
    Scenario: codex-agents-config-generation.SM1.AC3.existing_codex_config_is_preserved
      ...
      Then the existing Codex config content is preserved without creating repo-local Codex implementation files
