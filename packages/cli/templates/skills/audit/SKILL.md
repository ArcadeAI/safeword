...
This skill is required at the feature-ticket done-gate (ticket 147). The line below appends a current-run entry to `skill-invocations.log` under the project namespace root (`.project/`, or legacy `.safeword-project/` where that exists) so the done-gate hook can verify /audit was actually invoked. Claude Code expands the `!` line automatically and passes `${CLAUDE_SESSION_ID}` when available. The helper also resolves Claude remote-container ids from the runtime environment, and on Cursor the beforeShellExecution hook bridges the session id to the helper (the cursor-run-identity cache) — so on Claude and Cursor the fallback runs without hand-picking an id. Codex exposes its session id only to hooks and has no such bridge yet, so under Codex the fallback needs an id passed as its final argument. Hand-writing audit results cannot produce this feature-gate proof.
