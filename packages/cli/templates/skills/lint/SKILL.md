...
`/verify` owns the project's full lint, type-check, build, and test suites.
...
4. Only when the user explicitly asks for a **full** lint, run the repository's
   ...
   because implementation finished; `/verify` is the full closing gate.
...
4. Type errors reported by a targeted tool; remind the user that `/verify` owns
