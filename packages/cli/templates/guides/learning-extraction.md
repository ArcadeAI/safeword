...
**Project learnings** (`.project/learnings/[concept].md`):
...
**Historical archives** (`.project/learnings/archive/[bug-fix].md`):
...
2. Project `.project/learnings/` (project-specific)
...
1. **Before debugging** - Check if similar issue has learning already
   ...
   ```bash
   ls .project/learnings/*[technology]*.md
   ls .project/learnings/*[pattern]*.md
   ```
...
# Search by keyword
ls .project/learnings/*keyword*.md
```
...
→ Check: ls .project/learnings/*react*.md *hooks*.md *async*.md
...
→ Check: ls .project/learnings/*indexeddb*.md *safari*.md *quota*.md
→ Not found
...
→ Suggest extraction
→ Check: ls .project/learnings/*indexeddb*.md
...
│  ├─ YES → Continue
│  └─ NO → .project/learnings/archive/[bug-fix].md (optional)
│
├─ Choose destination:
│  │
...
│     └─ YES → Extract to: .project/learnings/[concept].md
...
### Forward-Looking Learning (.project/learnings/)
...
See `.project/learnings/archive/[investigation].md` for full debugging narrative.
```

### Debugging Narrative (.project/learnings/archive/)
...
After extracting to `.project/learnings/`, add cross-reference in SAFEWORD.md:

```markdown
## Common Gotchas

Project-specific gotchas in `.project/learnings/`:

- **Persistent UI Placement** - Controls in LayoutBar (always visible), not EditorTabBar (conditional) → `.project/learnings/persistent-ui.md`
- **Electron Renderer Context** - Renderer = browser, not Node.js; use `split(/[/\\]/)` for paths → `.project/learnings/electron-contexts.md`
...
### ✅ Project Learning (.project/learnings/)
...
**File:** `.project/learnings/persistent-ui.md`

**Cross-ref:** Link from `SAFEWORD.md` → Common Gotchas

---

### ❌ Archive (.project/learnings/archive/)
...
**File:** `.project/learnings/archive/electron-build-forgotten.md`
...
3. **Choose location** - Needs examples → `.project/learnings/race-conditions.md`
...
| Detailed gotcha       | `.project/learnings/` + SAFEWORD.md ref | Persistent UI, race conditions   |
| One-time bug          | `.project/learnings/archive/`           | Forgot to build before testing   |
...
.project/learnings/electron-gotchas.md

# BETTER (3 focused files)
.project/learnings/electron-renderer-context.md      (80 lines)
.project/learnings/electron-ipc-patterns.md         (60 lines)
.project/learnings/electron-path-validation.md      (50 lines)
```
