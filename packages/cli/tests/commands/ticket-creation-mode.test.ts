...
describe('resolveCreationMode (tracker-identity-and-join.TB1.AC1)', () => {
  ...
  // Epics stay local even with a tracker connected — a coordination container
  // (children[] over local folder ids), not off-boarded work. Identity does not
  // off-board for epics the way it does for patch/task/feature.
  it('routes --type=epic to local even with a configured provider', () => {
    expect(resolveCreationMode({ provider: 'github' }, { type: 'epic' })).toEqual({
      mode: 'local',
    });
  });

  // The epic-local rule wins over --issue too: an epic never adopts a tracker key.
  it('keeps --type=epic local even when --issue is supplied', () => {
    expect(resolveCreationMode({ provider: 'github' }, { type: 'epic', issue: 'ENG-9' })).toEqual({
      mode: 'local',
    });
  });
