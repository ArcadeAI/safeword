...
describe('createTicketRouted (tracker-identity-and-join.TB1)', () => {
  ...
  // Epics never off-board identity: with a provider configured, --type=epic still
  // mints locally, builds no tracker client, and writes no sidecar (children[] is
  // a local-folder reverse-index, so the epic's identity stays local).
  it('routes --type=epic to the local minter even with a provider configured', async () => {
    const buildWriter = vi.fn();
    const result = await createTicketRouted(
      cwd,
      { slug: 'big-rollout', type: 'epic' },
      {
        config: { provider: 'github', body: 'minimal', target: { repo: 'o/r' } },
        buildWriter,
        minter: fixedMinter('EPIC01'),
      },
    );

    expect(buildWriter).not.toHaveBeenCalled();
    expect(nodePath.basename(result.folderPath)).toBe('EPIC01-big-rollout');
    expect(existsSync(nodePath.join(cwd, '.safeword', 'tracker-map.json'))).toBe(false);
    // The local epic scaffold still lands (children[] frontmatter written).
    expect(readFileSync(nodePath.join(result.folderPath, 'ticket.md'), 'utf8')).toMatch(
      /^children:\s*\[\]$/m,
    );
  });
