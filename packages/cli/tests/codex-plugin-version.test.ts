...
describe('Codex plugin release contract', () => {
  ...
  it('accepts a cachebusted bundle identity without repair guidance', context => {
    if (!codexAvailable) context.skip();
    const root = nodePath.resolve(import.meta.dirname, '..');
    const repoRoot = nodePath.resolve(root, '../..');
  ...
  it('executes from the cache path reported by a real Codex plugin install', context => {
    if (!codexAvailable) context.skip();
    const root = nodePath.resolve(import.meta.dirname, '..');
    const repoRoot = nodePath.resolve(root, '../..');
