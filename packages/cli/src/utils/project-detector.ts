...
import { detect, SAFEWORD_PRIMARY_LINT_SCRIPT } from '../presets/typescript/detect.js';
...
function hasSafewordPrimaryLinter(
  cwd: string | undefined,
  scripts: Record<string, string>,
): boolean {
  return (
    cwd !== undefined &&
    existsSync(nodePath.join(cwd, '.safeword')) &&
    scripts.lint === SAFEWORD_PRIMARY_LINT_SCRIPT
  );
}
...
> {
  ...
  return {
    // Do not let the lint script generated by setup activate a conditional
    // schema branch on the next run. The installed marker distinguishes it
    // from an identical customer-owned script in an unconfigured project.
    existingLinter: hasExistingLinter(scripts) && !hasSafewordPrimaryLinter(cwd, scripts),
