/**
 * `safeword project codify` emitter (ticket CS86B0).
 *
...
export function emitVitestSkeleton(
  ...
  return emitVitestSkeletonFromScenarios(parseScenarios(testDefinitionsContent), options);
}

export function emitVitestSkeletonFromScenarios(
  scenarios: readonly ParsedScenario[],
  options: EmitOptions = {},
): string {
  const blocks = renderRuleBlocksFromScenarios(scenarios, (ruleName, ruleScenarios) =>
...
function renderRuleBlocks(
  ...
  return renderRuleBlocksFromScenarios(parseScenarios(testDefinitionsContent), renderRule);
}

function renderRuleBlocksFromScenarios(
  scenarios: readonly ParsedScenario[],
  renderRule: (ruleName: string, scenarios: readonly ParsedScenario[]) => string,
): string[] {
  return [...groupByRule(scenarios)].map(([ruleName, ruleScenarios]) =>
...
function buildHeader(source: string | undefined): string {
  const provenance =
    source === undefined ? '`safeword project codify`' : `\`safeword project codify ${source}\``;
  return [
    `// Generated by ${provenance}. One test per scenario in the ticket source.`,
