...
async function reviewPrHandler(invocation: CommandInvocation): Promise<CliResult> {
  const { resolvePrReviewConfig } = await import('../pr-review/config.js');
  const enabled = resolvePrReviewConfig(invocation.cwd).enabled;
  if (enabled && invocation.offline) return onlineRequired('review-pr');

  const { reviewPrCommand } = await import('../commands/review-pr.js');
  const outcome = await reviewPrCommand({
    projectDirectory: invocation.cwd,
    repository: stringOption(invocation.options, 'repository'),
    pull: stringOption(invocation.options, 'pull'),
  });
  return createResult({
    state: outcome.posted ? 'changed' : 'healthy',
    changed: outcome.posted,
    effects: {
      network: enabled
        ? [
            {
              kind: 'pr-review',
              target: 'GitHub',
              operation: outcome.posted ? 'read-write' : 'read',
            },
          ]
        : [],
    },
    findings: outcome.posted
      ? []
      : [{ code: 'PR_REVIEW_NOT_POSTED', message: outcome.reason, severity: 'info' }],
    data: { command: 'review-pr', outcome },
  });
}
