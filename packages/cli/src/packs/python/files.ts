...
import { detectPythonLayers, detectRootPackage, detectSolePackage } from './setup.js';
...
/**
 * Generic circular-import guard for a single-package project (ticket V4MATC,
 * upstream #847) — the Python analog of depcruise's no-circular rule, emitted
 * when no layer structure is detected but the top-level package is unambiguous.
 * Exact syntax per import-linter 2.x docs: `type = acyclic_siblings`
 * (underscore), `ancestors` is required, `root_package` singular is the
 * documented single-package form.
 */
function generateAcyclicImportLinterConfig(packageName: string): string {
  return `# Generated by safeword
# Circular-import guard - forbids dependency cycles between sibling modules
...
export const pythonManagedFiles: Record<string, ManagedFileDefinition> = {
  ...
  // Project-level import-linter config (created when no existing config and either
  // layers are detected or the top-level package is unambiguous — ticket V4MATC)
  '.importlinter': {
