...
- PR #471 update: release gate failed on dogfood parity only; synced `.safeword/hooks/stop-quality.ts` and `.safeword/hooks/cursor/stop.ts` to their prettier-wrapped template signatures.
- Quality-review: current published versions checked for Vitest, TypeScript, tsup, and Bun types; no new dependency or deprecated API issue in this PR. `bun audit --json` still reports existing unrelated low/moderate advisories.
- Refactor: cleaned a stale `quality-gates.test.ts` header comment that still said PostToolUse tracks TDD step transitions in state.
- Final verification: `bun run --cwd packages/cli test:release` pass; focused hook tests 30/30 pass; `quality-gates.test.ts` 77/77 pass; `bun run --cwd packages/cli typecheck` pass; `bun run lint:eslint` pass; `bun run lint:gherkin` pass; `git diff --check` pass.
