...
export function commandInvokesCloseoutCleanup(command: string): boolean {
  return splitShellSegments(command).some(segment => {
    const words = commandWords(segment);
    return isBunExecutable(words[0]) && isCloseoutCleanupPath(words[1]);
  });
}
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        id,
        ...(nonEmptyString(input.transcriptPath) === undefined
          ? {}
          : { transcriptPath: nonEmptyString(input.transcriptPath) }),
...
export function readFreshCloseoutBinding(
  ...
  try {
    ...
    return {
      ...
      id,
      ...(nonEmptyString(parsed.transcriptPath) === undefined
        ? {}
        : { transcriptPath: nonEmptyString(parsed.transcriptPath) }),
    };
  } catch {
    return undefined;
  } finally {
