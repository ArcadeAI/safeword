...
Scaffold `execution-plan.md` next to `ticket.md` from the installed
...
`execution-plan.md`. Use the host's Safeword project-knowledge resolver
to locate principles, personas, and surfaces. Include
...
Review kind: `plan-execution`. Target: `ticket-path/execution-plan.md`.
Context: the current spec, ticket, feature file, approved implementation plan,
resolved project knowledge, applicable guides, and Execution Plan template.
...
Use the host's Safeword review-stamp action for phase `plan-execution`, copying
the author, actual reviewer, independence, and review id from the approving
coordinator result.
