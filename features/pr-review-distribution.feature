@wip
Feature: PR review distribution — the runner's deterministically-checkable behavior

  Scenario source for ticket 36EEMY (pr-review-distribution). Split out of
  `autonomous-pr-review.feature` on 2026-07-19 so the runner's ledger actually
  binds: coverage resolves a ticket's scenarios as `<ticket-slug>.feature`, and
  the old filename was the parent EPIC's slug, so every entry was silently
  unenforced. The repo's convention is that CHILDREN own feature files and epics
  do not (epic Q4FX8Y holds only ticket.md; its children each carry the full set).
  ...
  The line that decides membership: a scenario belongs here only if it can FAIL
  for a runner reason. The runner drives the vendor through an injected spawn
  seam, so a scenario whose Given describes CODE SHAPE and whose Then asserts the
  model's JUDGMENT cannot fail here — stub the vendor and it asserts the stub
  (the tautological-mock antipattern; schema enforcement checks shape, not whether
  the model read the diff correctly). Five such Rules — R19 work type, R20 test
  coverage, TB2.R1 depth, TB2.R3 author-request — moved to the eval's holding pen
  in `autonomous-pr-review.feature` and are owed to CWGYH0. TB2.R2 stayed: its
  rows vary by an injectable concern-state, and it carries a rule R9 does not —
  an unresolved QUESTION, even with zero findings, forbids a `reviewed` receipt.
