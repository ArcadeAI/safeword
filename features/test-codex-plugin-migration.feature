Feature: Test Codex plugin migration
  ...
  Safeword's Codex migration is proven before it ships: Codex discovers the
  plugin, exposes scoped Safeword skills, runs packaged hook entrypoints, and
  ...
  Rule: test-codex-plugin-migration.TB1.R1 — A fresh repo can install and enable the Safeword Codex plugin without `.agents/skills` or repo-local `.safeword/hooks`

    Scenario: Fresh repo installs the plugin without repo-local Safeword implementation assets
      Given a fresh git repo with no Safeword Codex assets
      And an isolated CODEX_HOME configured with a local Safeword marketplace
      And the repo file tree has been recorded
      When the plugin install harness installs the Safeword Codex plugin
      Then `codex plugin list --json` reports the Safeword plugin as installed and enabled
      And the repo file tree is unchanged
      And the repo contains no Safeword-owned directories under `.agents`, `.codex`, `.safeword`, `.claude`, and `.cursor`
    ...
    Scenario: Invalid local marketplace fails before claiming plugin install success
      Given a fresh git repo with no Safeword Codex assets
      And an isolated CODEX_HOME configured with a local marketplace missing the Safeword plugin manifest
      When the plugin install harness tries to install the Safeword Codex plugin
      Then the install result names the plugin manifest validation failure
      And the repo still has no `.agents/skills` Safeword skill directory
      And the repo still has no `.safeword/hooks/codex` directory

  @test-codex-plugin-migration.TB1.R2 @surface.openai-codex
  Rule: test-codex-plugin-migration.TB1.R2 — Safeword Codex skills are visible to the agent from the plugin with stable, intentional invocation names

    Scenario: Plugin skills expose the approved scoped invocation names
      Given a fresh repo with the Safeword Codex plugin installed and enabled
      And the repo has no repo-local Safeword skills
    ...
    Scenario: Bare-name compatibility shims are treated as repo-residue
      ...
      Then no Safeword-owned `.agents/skills/bdd/SKILL.md` alias exists
      And no Safeword-owned `.agents/skills/verify/SKILL.md` alias exists
  ...
  Rule: test-codex-plugin-migration.TB1.R3 — Safeword Codex hooks execute the packaged CLI entrypoints and preserve the existing deny, allow, context, and continuation semantics
    ...
    Scenario: PreToolUse denial runs through the packaged CLI entrypoint
      ...
      Then the hook output denies the edit with the existing Safeword phase-gate reason
      And the denial tells the Codex user to run the Safeword explain guidance
    ...
    Scenario: PostToolUse additional context runs through the packaged CLI entrypoint
      ...
      And the additional context contains the fixture's Safeword guidance line
    ...
    Scenario: UserPromptSubmit additional context runs through the packaged CLI entrypoint
      Given the Safeword package has been packed and installed into a fixture project
      And the fixture has queued Safeword prompt context for Codex
    ...
    Scenario: Stop continuation runs through the packaged CLI entrypoint
      Given the Safeword package has been packed and installed into a fixture project
      And the fixture has a Codex stop payload that should produce a Safeword continuation
      ...
      And the continuation reason contains the fixture's Safeword continuation message

    @rejection
    Scenario: Plugin hook commands never point at repo-local hook scripts
      Given the Safeword Codex plugin hook manifest
      When the hook commands are inspected
      Then no command contains `.safeword/hooks`
      And no command depends on `git rev-parse --show-toplevel` to find Safeword hook code
      And each command invokes a packaged Safeword command entrypoint

  @test-codex-plugin-migration.TB1.R4 @surface.openai-codex
  Rule: test-codex-plugin-migration.TB1.R4 — Installing Codex for an old project leaves user-owned project data and fallback assets intact until an explicit proven handoff
    ...
    Scenario: Unavailable profile enrollment preserves an old project's authored data and fallback
      ...
      And Safeword keeps repo-local `.agents/skills` available during the compatibility window
      And legacy Safeword Codex hook runtime files remain until explicit handoff cleanup
    ...
    Scenario: Successful handoff preserves authored project data while removing managed fallback
      ...
      When the plugin migration install runs with profile enrollment available
      Then Safeword is enrolled in the Codex profile
      And the user-owned tickets and learnings remain byte-identical
      And the user-authored skill remains byte-identical
      And Safeword-owned Codex skill files are removed after finalization
      And legacy Safeword Codex hook runtime files are removed after finalization
    ...
    Scenario: User-authored Codex skills survive the migration
      ...
      And Safeword-owned Codex skill files remain beside the user-authored skill until finalization

    @rejection
    Scenario: Customized Codex config is not clobbered while stale Safeword hooks await explicit migration
      Given an old project-local Codex install with user-authored Codex config entries
      And the config also contains old Safeword hook commands pointing at `.safeword/hooks/codex`
      When the plugin migration install runs without profile enrollment available
      Then the user-authored Codex config entries remain
      And the stale Safeword project-local hook commands remain until explicit migration
  ...
  Rule: test-codex-plugin-migration.SM1.R1 — Static and release checks prove the plugin manifest, marketplace entry, bundled files, and packed package contents are valid
    ...
    Scenario: Release check proves the packed package contains the plugin and hook entrypoints
      Given the Safeword package has been packed from the working tree
      When the release contract inspects the package contents
      Then the package contains the Safeword Codex plugin manifest
    ...
    Scenario: Missing packaged hook dependency blocks the release contract
      Given the Safeword package omits a helper required by a Codex hook entrypoint
  ...
  Rule: test-codex-plugin-migration.SM1.R2 — An isolated plugin install harness proves Codex can discover, install, enable, and expose the Safeword plugin under a temp CODEX_HOME

    Scenario: Plugin install harness mutates only the isolated Codex home
      Given a temporary CODEX_HOME with no installed Safeword plugin
      And the developer's real Codex home has a recorded plugin list
      When the isolated plugin install harness runs
      Then the temporary CODEX_HOME contains the Safeword plugin install state
    ...
    Scenario: Disabled plugin is reported as unavailable to the prompt surface
      Given a temporary CODEX_HOME where the Safeword plugin is installed but disabled
      When the prompt surface is inspected with `codex debug prompt-input`
  ...
  Rule: test-codex-plugin-migration.SM1.R3 — Deterministic hook contract tests exercise exact Codex hook JSON against packaged CLI entrypoints without involving the model
    ...
    Scenario: Malformed Codex hook input fails open through the packaged entrypoint
      Given a packaged Safeword Codex hook entrypoint
      ...
      And the Safeword self-report spool remains unchanged
  ...
  Rule: test-codex-plugin-migration.SM1.R4 — A single opt-in live Codex smoke proves real `codex exec` invokes the installed plugin hooks, while model-mediated cost and flake stay out of default verification
    ...
    Scenario: Opt-in live smoke observes a plugin-installed hook denial
      ...
      And a fresh repo has the Safeword Codex plugin installed, enabled, and live-authenticated under an isolated CODEX_HOME
      When `codex exec --json --dangerously-bypass-hook-trust` attempts a supported edit that violates a Safeword gate
      Then the JSONL output contains the Safeword hook denial
    ...
    Scenario: Untrusted plugin hooks are reported as not active for normal Codex runs
      Given a fresh repo has the Safeword Codex plugin installed and enabled under an isolated CODEX_HOME
      And the Safeword plugin hooks have not been trusted in Codex
      When the plugin verification runs without `--dangerously-bypass-hook-trust`
      Then the verification reports that Safeword plugin hooks require Codex hook trust review
      And it does not claim Safeword edit gates are active for normal Codex runs
