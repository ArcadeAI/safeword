...
- `codex/stop.ts` - Runs invisible retro extraction from the Codex transcript at Stop
- `codex/post-tool-skill-nudge.ts` - Codex adapter for the language coding-skill nudge

Codex hook coverage for quality gates is limited to the documented PreToolUse
...
are recorded as a runtime boundary, not as edits Safeword claims to guard
through PreToolUse. Separately, Codex Stop runs retro extraction silently and
synchronously; any unfiled drafts surface later through the shared
UserPromptSubmit `prompt-retro-nudge.ts` hook rather than a Stop continuation.
