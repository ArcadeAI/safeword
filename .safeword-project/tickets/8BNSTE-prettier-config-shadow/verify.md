...
Pinned to commit `35942a02` (fix: exact-match detection). Behavior landed across `412005e3` (GREEN) + `35942a02` (quality-review fix). Verified on branch `arcade-pipeline-sync`.

## Verify Checklist

**Test Suite:** ✓ 2314/2314 tests pass (1 skipped, unrelated; 140 files; `npx vitest run` from `packages/cli/`)
...
**Quality review:** Caught a prefix-match false-positive (`.prettierrc.bak` would suppress install); fixed to exact filename-set matching in `35942a02`. Verified detection set against prettier 3.8.3 docs and the doc-verified `lint-config.ts` list (commit ffc1ad58).
...
- **Exact-match, no prefix false-positive.** `detect.test.ts`: false for `.prettierrc.bak` and `prettier.config.js.disabled` (disabled/backup files prettier won't load must not suppress install). Uses the exact filename set mirroring `lint-config.ts` (ticket 1J6JKP), not a `.prettierrc*` prefix. PASS.
