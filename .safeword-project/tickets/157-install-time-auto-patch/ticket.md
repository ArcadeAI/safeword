...
scope:
  ...
  - Idempotency — if the config text already contains the substring `vendoredIgnores`, skip the patch entirely (covers re-runs after either auto-patch or manual 156-nudge application)
  - Safety — write `<config-path>.safeword-bak` before any edit; if a post-edit `node --check` fails, restore from backup and fall back to 156's print-nudge. TypeScript variants (`.ts/.mts/.cts`) skip `node --check` (the textual insert is trusted; a TS-aware checker isn't worth the dep weight for a one-line insert)
  ...
  - Opt-out — `--no-modify` flag on `setup` and `upgrade` AND `SAFEWORD_NO_MODIFY=1` env var both skip auto-patch entirely and fall through to 156's print-nudge
...
out_of_scope:
  - Legacy `.eslintrc.*` configs (json/yaml/js/cjs) — these projects fall straight through to 156's print-nudge; auto-patch for them is its own deferred problem
...
done_when:
  ...
  - `--no-modify` flag and `SAFEWORD_NO_MODIFY=1` env var each independently short-circuit the auto-patch and fall through to 156's print-nudge with the consumer's config untouched
  - Every bail-to-print trigger (defineConfig-but-no-inner-array, function-returning-config, single-imported-config, syntax-check fail after edit, read/write fail) leaves the consumer's config exactly as it was AND prints 156's nudge
...
**Goal:** Make the 156 print-nudge moot for the common case. Most users won't read printed instructions and many don't know what ESLint is — so safeword should just add the line itself, with a backup, and tell the user it did. Print-nudge becomes the fallback for legacy configs and unrecognized patterns rather than the primary experience.

**Why:** Ticket 156 shipped the export and a print-only nudge. The user explicitly flagged that print-only is weak for non-technical users; the 84 false-positive lint errors they're hitting will persist if they don't read or act on the message. Auto-patch closes that gap.
...
- **Idempotency by substring.** A config that contains `vendoredIgnores` anywhere is treated as already-patched. This avoids needing state outside the file itself, and handles both auto-patch and manual-156 application identically.

## References

- [Ticket 156](.safeword-project/tickets/156-vendored-ignores-export/ticket.md) — `vendoredIgnores` export + print-nudge (foundation)
- [vendored-ignores-nudge.ts](packages/cli/src/utils/vendored-ignores-nudge.ts) — 156's print-nudge helper; auto-patch's fallback target
