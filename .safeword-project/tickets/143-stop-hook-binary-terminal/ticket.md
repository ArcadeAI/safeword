...
- 2026-05-15T05:35:00Z Post-merge review (before PR #91 merged) revealed criteria loss: the binary form's per-phase evidence sentences had over-simplified, dropping checks from legacy prompts (failure modes + open questions on intake; correctness/simplicity/docs-alignment on implement; scope drift + scenario coverage + cross-scenario refactoring on done). Restoring all dropped criteria. Also lifting universal critical-review (correctness/simplicity/latest-docs) from implement into the universal header (applies to all phases per user direction). REFACTOR enhanced with refactor-skill iron law (one refactoring at a time, smell-named, no behavior change). Research instruction expanded to specify depth-matching: code/docs for syntax; primary literature (peer-reviewed/lab/preprints) for design/empirical claims; blog posts/tweets/marketing don't count. Quality test count: 13 → 22 (added 9 new scenarios across 3 new rules). Marker contract unchanged so 144's parity check still passes.
- 2026-05-15T05:55:00Z Propulsive verdicts iteration (`7f86949`): added `Next: <one concrete action>` field to CONFIDENT (research-aligned via Anthropic "Building Effective Agents" + production agent frameworks like HumanLayer); added optional Meanwhile parenthetical to BLOCKED for parallel work that doesn't depend on the blocker. Quality tests: 22 → 25.
- 2026-05-15T06:10:00Z Methodology encoding (`3be4dcf`): encoded user's standard decision-making methodology (investigate → enumerate options → debate against correctness/elegance/no-bloat → recommend) into universal header. Research-aligned: Tree of Thoughts (Yao 2023), Chain-of-Thought (Wei 2022), Self-Consistency (Wang 2022). Catches "premature BLOCKED" failure mode. Sharpened BLOCKED template to require "a question with a falsifiable answer" — catches the "not sure, please review" gaming pattern. Quality tests: 25 → 27.
- 2026-05-15T06:12:00Z Regression guard (`41fa387`): parameterized test iterating all 10 phase variants and asserting universal header elements appear in each. Locks in "applies to all phases" invariant. Quality tests: 27 → 38.
- 2026-05-15T06:29:00Z Spec-vs-implementation contract (`9ef843a`): customer trace (Nate) surfaced agent conflating implementation choices with spec interpretation; agent proposed inventing permissions and escalated impl paths to user. Encoded user's contract — "Implementation choices are yours to make and own. BLOCKED is for spec, scope, or value decisions that require human input." Quality tests: 38 → 40.
- 2026-05-15T06:30:00Z Final wrap. 6 iterations, 8 commits total on PR #91. Quality test count 13 → 40. verify.md refreshed with full iteration history. Two follow-up gaps surfaced from customer trace deferred to separate tickets: `/verify` output-shape (next ticket), test-pinning-bugs catch (defer unless recurs). Phase: done (unchanged); status: done (unchanged).
