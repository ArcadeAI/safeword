...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| Root resolution source           | config `paths.projectRoot` set · `.project/` present (no config) · only `.safeword-project/` present · **both present** (boundary) · **neither present** (boundary, fresh) | SM1.AC1, DEV1.AC2    |
| Configured-root path type        | relative (resolve against cwd) · absolute (verbatim, boundary)                                                                                                             | DEV2.AC1             |
| Per-file override interaction    | no per-file override (derive from root) · per-file `paths.X` set (wins for that file)                                                                                      | DEV2.AC2             |
| Default-subpath derivation       | personas · glossary · architecture — each default = `<resolved-root>/<file>.md`                                                                                            | DEV1.AC1             |
