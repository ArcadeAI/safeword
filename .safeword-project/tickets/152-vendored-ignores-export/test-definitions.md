...
- [ ] **Given** a consumer imports `safeword from 'safeword/eslint'`
...
- [ ] **Given** `safeword.configs.vendoredIgnores`
...
- [ ] **Given** a fixture project layout with `src/app.ts`, `.safeword/hooks/foo.ts`, and `.dependency-cruiser.cjs` (each containing a deliberate `no-console` violation), and an `eslint.config.mjs` written as
...
- [ ] **Given** a project with a pre-existing `eslint.config.mjs` and a `package.json` declaring a JavaScript dependency
...
- [ ] **Given** a JavaScript project with NO pre-existing ESLint config
      **When** the user runs `safeword setup`
...
- [ ] **Given** a Python-only project with no detected `javascript` language layer, even if a stray `.eslintrc` exists from an unrelated tool
      **When** the user runs `safeword setup`
...
- [ ] **Given** an already-initialized safeword project whose `eslint.config.mjs` was authored by the user (not safeword)
...
- [ ] **Given** an already-initialized safeword project whose `eslint.config.*` contains the substring `.safeword/` anywhere in its text (whether via safeword's own `getIgnores()` output, an explicit user-added ignore, or a prior application of the snippet itself)
      **When** the user runs `safeword upgrade`
      **Then** the command output does not contain the `vendoredIgnores` snippet
...
- [ ] **Given** the rewritten ESLint-config regex in `pre-tool-config-guard.ts`
...
- [ ] **Given** the rewritten Prettier-config regex in `pre-tool-config-guard.ts`
...
- [ ] **Given** the rewritten line in `.safeword/hooks/cursor/stop.ts`
...
- [ ] **Given** the rewritten cleanup catch
...
- [ ] **Given** edits to `.safeword/hooks/pre-tool-config-guard.ts` and `.safeword/hooks/cursor/stop.ts`
