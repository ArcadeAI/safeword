...
- **Closes by `status`, not `phase: done`.** Two epics (DZ2NM5, P8RJ4M) were closed by setting `status: done` with `phase` left at `intake`, so the `phase: done` done-gate (tests + verify.md + skill-invocation) never fired. verify.md was hand-written until a retro-run was explicitly requested. The gate is trivially sidesteppable for any ticket that doesn't ride the phase machine.
