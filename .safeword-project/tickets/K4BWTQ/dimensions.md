...
**Pruning:** section-state × location-presence full cross (3×2) sampled at the three decision cells: content+absent (flags), skip+absent (clean), content+present (clean). skip+present (stale skip) deliberately unflagged — checking skip-reason text against location state is prose interpretation, out of scope per the structural-only ruling. Directory-content naming variants collapse into one accept-any scenario plus the README exclusion.
