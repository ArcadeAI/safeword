...
- **Build order:** (1) ✓ pure relevance fn; (2) ✓ surface-decision fn; (3) git gathering (`git --since=<last_modified>` + ticket-path extraction) + session-HEAD marker + wire into UserPromptSubmit hook + heads-up injection; (4) skill prose for the agent investigation.
