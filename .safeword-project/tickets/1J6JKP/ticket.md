...
scope:
  - Fix `session-lint-check.ts` config detection to **prefix-match** instead of enumerating filenames — present if a project dir entry starts with `eslint.config.` / `.eslintrc` (eslint) or `.prettierrc` / `prettier.config.` (prettier). Covers all current + future extensions (the drift that caused this ticket).
...
out_of_scope:
  ...
  - Prettier config living in the package.json `"prettier"` key (no config file) — a rare edge; file-prefix detection is the live bug. Tiny follow-up if it ever bites.
done_when:
