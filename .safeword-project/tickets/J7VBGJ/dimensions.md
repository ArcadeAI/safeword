...
| TDD step at commit time              | REFACTOR (gate fires) / RED, GREEN, unknown (gate does nothing — see scope-reduction note below)                      | scope (commit-time hook)         |
| Staged file shape at REFACTOR        | only non-test files (allow) / includes any test file (block)                                                          | scope (REFACTOR file-path rule)  |
...
- **RED-step commit file-path restriction** — originally in scope, dropped 2026-05-21 after revisiting `.safeword-project/learnings/procedural-gates-generalize-beyond-tdd.md`. The TDAD finding (arxiv 2603.17973v2) that generic procedural rules degrade agent performance applies directly. The rule was not load-bearing — commit bundling is already caught by SHA-distinctness at done; the REFACTOR file-path rule is the only file-path rule that catches something the SHA mechanism cannot (test-behavior drift during cleanup).
