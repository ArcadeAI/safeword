...
function runCodexPluginCommand(world: CodexPluginMigrationWorld, args: string[]): CommandResult {
  return runCommand('codex', args, {
    cwd: requirePath(world.codexPluginRepoRoot, 'fresh repo root'),
    env: {
      CODEX_HOME: requirePath(world.codexPluginCodexHome, 'isolated CODEX_HOME'),
    },
  });
}
...
function installSafeWordCodexPlugin(world: CodexPluginMigrationWorld): void {
  const marketplaceRoot = requirePath(world.codexPluginMarketplaceRoot, 'local marketplace root');
  const marketplacePreparation = prepareMarketplacePlugin(marketplaceRoot);
  if (marketplacePreparation) {
    world.codexPluginInstallResult = marketplacePreparation;
    return;
  }

  if (recordMarketplaceValidationFailure(world, marketplaceRoot)) return;

  const addMarketplaceResult = runCodexPluginCommand(world, [
    'plugin',
    'marketplace',
    'add',
    marketplaceRoot,
  ...
  if (addMarketplaceResult.exitCode !== 0) {
    world.codexPluginInstallResult = addMarketplaceResult;
    return;
  }

  const installResult = runCodexPluginCommand(world, [
    'plugin',
    'add',
    'safeword',
    '--marketplace',
  ...
  world.codexPluginInstallResult = installResult;
  world.codexPluginInstallSummary = summarizePluginInstallResult(installResult);

  if (installResult.exitCode === 0) {
    world.codexPluginListResult = runCodexPluginCommand(world, ['plugin', 'list', '--json']);
  }
}
...
function installSafeWordPluginFixture(
  ...
  installSafeWordCodexPlugin(world);
...
When(
  ...
  function (this: CodexPluginMigrationWorld) {
    ...
    const addMarketplaceResult = runCodexPluginCommand(this, [
      'plugin',
      'marketplace',
      'add',
      marketplaceRoot,
    ...
    this.codexPluginInstallResult = runCodexPluginCommand(this, [
      'plugin',
      'add',
      'safeword',
      '--marketplace',
...
When(
  ...
  function (this: CodexPluginMigrationWorld) {
    this.codexPluginHookResult = runPackagedCodexHook(
      requirePath(this.codexPluginRepoRoot, 'repo root'),
      'pre-tool-use',
      {
        hook_event_name: 'PreToolUse',
...
When(
  ...
  function (this: CodexPluginMigrationWorld) {
    ...
    this.codexPluginHookResult = runPackagedCodexHook(repoRoot, 'session-start', {
      hook_event_name: 'SessionStart',
      session_id: 'codex-test-session',
    });
  },
...
When(
  ...
  function (this: CodexPluginMigrationWorld) {
    ...
    this.codexPluginHookResult = runPackagedCodexHook(repoRoot, 'post-tool-use', {
      hook_event_name: 'PostToolUse',
      session_id: 'codex-test-session',
      tool_name: 'Edit',
      tool_input: { file_path: 'src/generated.ts' },
...
When(
  ...
  function (this: CodexPluginMigrationWorld) {
    ...
    this.codexPluginHookContractResults = commands.map(command => {
      ...
      const result = runPackagedCodexHook(repoRoot, commandName, fixture.input);
...
When('the entrypoint receives malformed JSON on stdin', function (this: CodexPluginMigrationWorld) {
  ...
  this.codexPluginHookResult = runPackagedCodexHook(repoRoot, commandName, '{not valid json');
});
 ...
 * unavailable, and each one asserts the loud attention it produces (#1973).
 * That signal proves the upgrade ran, reached the Codex handoff, and preserved
 * the project on the way out without blocking work. Without it these
 * scenarios pass whether or not anything executed, because untouched files
 * look identical to files nothing reached.
 *
 * An invalid isolated CODEX_HOME makes enrollment fail without hiding the
 * other process tools the upgrade needs or mutating the ambient Codex profile.
 */
When(
