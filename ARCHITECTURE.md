...
**Superseded in part by:** [User-Private Project Contexts Beneath Explicit Repository Enrollment](#user-private-project-contexts-beneath-explicit-repository-enrollment) (user-private fallback state; repository mutation still requires explicit consent)
...
### User-Private Project Contexts Beneath Explicit Repository Enrollment

**Status:** Proposed
**Date:** 2026-09-13
**Supersedes in part:** [Explicit Project Enrollment for Profile-Scoped Codex Hooks](#explicit-project-enrollment-for-profile-scoped-codex-hooks)

| Field          | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Context        | Profile-delivered Safeword workflows can have valuable project work before a repository is enrolled: BDD artifacts, project knowledge, mutable execution state, and invocation proof. Stopping or silently dropping that work exposes internal errors and loses continuity, while treating it as permission to modify the repository violates the explicit-enrollment boundary. Linked Git worktrees additionally need shared project knowledge without sharing mutable operation state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Decision       | Before the first Safeword-owned project-state access, resolve one authoritative context. An active current-repository context wins; an existing exact global partition wins over an unrelated enrolled ancestor; otherwise native host input may select a containing project, reviewed local installation, or automatic global storage. Any non-acceptance of repository setup uses an owner-private global partition without a second prompt. Git's canonical common directory identifies shared knowledge and its canonical worktree Git directory identifies mutable state; non-Git directories use canonical-path identity. Global directory names contain hashes, not source paths. A verified local installation shadows global as a whole and never writes through. Later installation hydrates compatible global data through the canonical reviewed plan, writes local activation last, and never deletes or updates the preserved global snapshot. |
| Consequences   | Stateful workflows continue in unenrolled repositories without gaining repository authority. Unrelated checkouts are isolated; linked worktrees share knowledge and isolate mutable state. Missing local overlays fall back to the preserved pre-install snapshot, but post-install local changes are intentionally absent from that snapshot. Hydration conflicts, unreadable sources, permission failures, and failed verification stop with recovery instead of silently overwriting or activating partial local state. Repository moves may select a new partition until demonstrated demand justifies an explicit relink operation.                                                                                                                                                                                                                                                                                                                     |
| Alternatives   | Stop or degrade statelessly: rejected because required workflows lose work and expose framework internals. One hash per checkout: rejected because linked worktrees cannot share project knowledge. A generated project-ID registry: rejected because it adds relinking, collision, and registry-recovery state before move continuity is required. Per-file union fallback or local-to-global write-through: rejected because stale global values can leak into local authority and the preserved snapshot would no longer be stable.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Reassess when  | Git's public common/worktree discovery changes; arbitrary repository moves become a demonstrated need; a supported host cannot prove builder input; platform owner-only storage cannot be established; or the existing lifecycle plan cannot represent hydration and activation atomically enough to preserve the stated authority boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Implementation | Ticket `EM9ZSV`; `packages/cli/src/project-context/`, lifecycle hydration in `packages/cli/src/lifecycle/project-install.ts` and `packages/cli/src/reconcile.ts`, schema/catalogue parity, native host adapters, and `packages/cli/features/quiet-unenrolled-reviews.feature`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
