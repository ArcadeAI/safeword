...
| Field          | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| What           | BDD Phase 0 (`intake`) now writes a per-ticket `spec.md` with persona-anchored Jobs To Be Done → Rules → engineering scope, backed by a project glossary and personas file. Scenarios carry lineage `<slug>.<persona><JTBD#>.R<#>.<scenario>` so coverage gaps are machine-checkable. (Converged from two criteria names — "Acceptance Criteria" and "Rule" — to the single Rule tier in ticket 1SVCB9 / issue #716; legacy `.AC<n>` is a soft-deprecated alias, `safeword migrate-ac` migrates it.) |
| Why            | Engineering scope (`scope` / `out_of_scope` / `done_when`) captured _what_ to build but not _who_ for or _why_; product framing anchors scenarios to verifiable Rules and lets `safeword check` flag uncovered Rules and orphan scenarios.                                                                                                                                                                                                                                                           |
