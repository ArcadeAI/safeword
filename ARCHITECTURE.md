...
| Consequences   | All class-1 skills share one observable execution and fallback contract. A same-agent fallback is labeled degraded and cannot satisfy required cross-agent evidence. Missing installation/authentication, runtime failure/timeout, and invalid output advance through the bounded CLI routes. Direct CLI use still returns typed `REVIEW_ROUTES_EXHAUSTED` loudly. Foreground agent workflows may then make one best-effort host-native fresh-context attempt and one main-thread self-review; both remain explicitly non-independent, never create a review stamp, and preserve a required-policy failure. Neutral packet containment, tool denial, vendor sandbox flags, and post-run hashes remain layered controls; no sandbox label is trusted alone. |
