...
| Decision       | Publish `safeword/pr-readiness` from a job in the existing router: no checkout, no model, no secret, `statuses: write` only. It reads the pull request body and head SHA and emits one of five verdicts — `current`, `draft`, `stale`, `missing`, `blocked` — whose status descriptions come from a closed set of constants, so attacker-controlled body text never reaches the privileged request. It observes the evidence block and never judges the gates behind it. The job is serialized per pull request with `cancel-in-progress`, so the newest event's view wins and a delayed run cannot publish a stale verdict over a newer one. `edited` is in the trigger set because the status is about the body; the advisory reviewer excludes that action so a body edit never spends a model call. Safeword never modifies branch protection, and **recommends against requiring this context.** Known limitation, accepted: a commit status is commit-scoped while readiness is pull-request-scoped, so two open pull requests sharing one head commit — the same branch opened against two base branches — write to the same context, and the later run's verdict stands. Serializing on the head SHA removes the race but not the ambiguity; a conservative cross-pull-request verdict would cost an extra listing call and a second evaluation pass for a rare topology. This is a further reason not to require the context. |
